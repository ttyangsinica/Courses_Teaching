...
\begin{columns}[T]
	\begin{column}{0.5\textwidth}
		\textbf{Setup: job training program}
		\vspace{0.15cm}
		\begin{itemize}
			\item \textbf{Treatment group} ($D_i=1$): workers who received training
			\vspace{0.1cm}
			\item \textbf{Control group} ($D_i=0$): workers who did not
			\vspace{0.1cm}
			\item \textbf{Outcome} $Y_i$: wage after the program
		\end{itemize}
		\vspace{0.3cm}
		\textbf{Question:} does the training effect differ by worker type?
		\vspace{0.15cm}
		\begin{itemize}
			\item Young workers may gain more than older workers
			\item Less-educated workers may benefit differently
		\end{itemize}
	\end{column}
	\begin{column}{0.5\textwidth}
		\textbf{Idea: split workers into groups (leaves)}
		\vspace{0.15cm}
		\begin{itemize}
			\item Within each leaf $\ell$, compare treated vs.\ control:
			\[
				\hat{\tau}(\ell) = \underbrace{\bar{Y}^{1}_\ell}_{\text{treated avg}} - \underbrace{\bar{Y}^{0}_\ell}_{\text{control avg}}
			\]
			\item Workers in the same leaf have \textbf{similar} $X$ (age, educ, \ldots)
			\vspace{0.1cm}
			\item Under \textbf{CIA}: within the leaf, treatment is ``as good as random'' $\Rightarrow$ \textbf{local experiment}
		\end{itemize}
		\vspace{0.15cm}
		\small Goal: find leaves where $\hat{\tau}(\ell)$ \textbf{differs the most}
	\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{Causal Tree}{Finding the Best Split}
	\begin{itemize}
		\item At each node, try every variable $j$ and threshold $s$; choose the split that makes \textbf{treatment effects most different} across the two groups:
		\[
			\Delta(j,s) = |\hat{\tau}(R_1) - \hat{\tau}(R_2)|, \qquad
			\hat{\tau}(R) = \bar{Y}^{1}_{R} - \bar{Y}^{0}_{R}
		\]
	\end{itemize}
	\vspace{0.1cm}
	\textbf{Example:} 10 workers, treatment = job training ($D_i=1$: trained, $D_i=0$: not trained)
	\vspace{0.1cm}
	\begin{center}
	\small
	\begin{tabular}{lp{2.6cm}p{2.6cm}c}
		\hline
		\textbf{Split} & $\hat{\tau}(R_1)$ \newline (treated$-$control in $R_1$) & $\hat{\tau}(R_2)$ \newline (treated$-$control in $R_2$) & $\Delta$ \\
		\hline
		Age $< 30$  & \$2{,}500 & \$400   & \textbf{\$2{,}100} $\leftarrow$ \\
	...
	\vspace{0.1cm}
	\begin{itemize}
		\item \textbf{Age $< 30$ is selected}: young workers (treated$-$control = \$2,500) benefit far more than older workers (\$400)
		\item A Decision Tree would split on whichever variable best \textit{predicts wages} --- entirely different goal
	\end{itemize}
\end{frame}
...
\begin{frame}{Causal Tree}{The Bias Problem}
	\begin{itemize}
		\item After finding the split that maximises $\Delta(j,s)$, we compute $\hat{\tau}(\ell)$ \textbf{using the same data}
		\vspace{0.2cm}
		\item This creates \textbf{adaptive bias}: the tree searched many possible splits and picked the largest difference --- the estimates are inflated
		\vspace{0.2cm}
	...
	\begin{center}
	\begin{tikzpicture}
		\node[draw, rounded corners, fill=red!10, text width=8cm, align=center, font=\small, inner sep=8pt] {
...
\begin{frame}{Honest Estimation}{Athey \& Imbens (2016)}
	\begin{itemize}
		...
		\vspace{0.15cm}
		\begin{itemize}
			\item Each observation is assigned to a leaf using the tree from Step 1
			\vspace{0.15cm}
...
\begin{columns}[T]
	...
	\begin{column}{0.48\textwidth}
	\small
	\textbf{Reading the causal tree:}
	\vspace{0.2cm}
	\begin{itemize}
		\item \textbf{Leaf 1} (age $< 30$):\\
			Young workers gain \$2,500 from training
		\vspace{0.15cm}
		\item \textbf{Leaf 2} (age $\geq 30$, educ $< 12$):\\
			Older, less-educated gain only \$400
		\vspace{0.15cm}
		\item \textbf{Leaf 3} (age $\geq 30$, educ $\geq 12$):\\
			Older, educated workers gain \$1,600
	\end{itemize}
	\vspace{0.2cm}
	Each worker $i$ is assigned $\hat{\tau}(x_i) = \hat{\tau}(\ell)$ based on their leaf
	\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{Causal Forest: Intuition}
	\begin{itemize}
		\item How does the forest estimate $\hat{\tau}(x)$ for a specific person $x$?
		\vspace{0.3cm}
		\item \textbf{Step 1: Find ``neighbors''} --- identify individuals who look similar to $x$
		\vspace{0.15cm}
		\begin{itemize}
			\item Across the $B$ trees, person $x$ falls into different leaves
			\vspace{0.1cm}
			\item Any individual $i$ who frequently ends up in the \textit{same leaf} as $x$ is considered a \textbf{neighbor} of $x$
			\vspace{0.1cm}
			\item Neighbors are people with \textbf{similar} age, education, experience, \ldots
		\end{itemize}
		\vspace{0.3cm}
		\item \textbf{Step 2: Run a local experiment} --- among $x$'s neighbors, compare treated vs.\ control
		\vspace{0.15cm}
		\begin{itemize}
			\item Under CIA, neighbors are comparable $\Rightarrow$ difference in outcomes $\approx$ causal effect
			\vspace{0.1cm}
			\item The estimate $\hat{\tau}(x)$ reflects the treatment effect \textbf{for people like $x$}
		\end{itemize}
		\vspace{0.3cm}
		\item[$\Rightarrow$] Causal Forest = running many \textbf{local experiments}, one for each type of person
	\end{itemize}
\end{frame}
