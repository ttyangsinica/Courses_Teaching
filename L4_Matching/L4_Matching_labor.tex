...
From the previous example, suppose \textbf{age} is the only confounding factor in evaluating the effect of job training:

\vspace{0.2cm}
\begin{itemize}
    \item Older workers are \emph{more likely to seek job training}
    \vspace{0.15cm}
    \item Older workers \emph{also earn more} due to their experience
\end{itemize}

\vspace{0.3cm}
$\Rightarrow$ A naive comparison mixes up the \textbf{effect of training} with the \textbf{effect of age}.

\vspace{0.3cm}
\begin{alertblock}{Solution}
\textbf{Match by age:} Once we compare people of the \textbf{same age}, the confounding effect of age disappears — treatment assignment becomes \textbf{``as good as random''} within each age group.
\end{alertblock}
...
% %% ── 第四頁：視覺總結（2×2 表格）─────────────────────────────────────────────
% \begin{frame}{Conditional Independence Assumption}{Summary: Observable vs.\ Counterfactual}
...
\begin{frame}{Approximate Matching}{}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\vspace{0.2cm}
			\item[2] \textbf{Propensity Score Matching:} match on likelihood of being treated
            \vspace{0.2cm}
			\item[3] \textbf{Coarsened Exact Matching:}	match within coarsened subgroups
...
\vspace{0.3cm}
\begin{itemize}
    \item Strata 1, 4, 5: both treated and control units present
