...
3 & \textbf{Label \& clean data}
  & Label variables/values, handle missing values and outliers \\
4 & \textbf{Descriptive statistics}
  & \texttt{summarize}, \texttt{tabulate}, \texttt{inspect} \\
5 & \textbf{Pre-analysis balance check}
  & Compare treated vs.\ control \emph{before} any matching \\
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \vspace{0.1cm}
        \item \texttt{/do} --- all \texttt{.do} files
        \vspace{0.1cm}
        \item \texttt{/output} --- tables, figures, logs
    \end{itemize}
    \vspace{0.2cm}
    \item Use \textbf{relative paths} or a \textbf{path setup block}
    at the top of every \texttt{.do} file (as in \texttt{matching.do})
    so the project runs on any machine
    \vspace{0.2cm}
    \item \textbf{Never overwrite raw data} --- all cleaning steps should
    be recorded in a \texttt{.do} file and saved to \texttt{/workdata}
    \vspace{0.2cm}
...
\begin{itemize}
    ...
    \vspace{0.1cm}
    \begin{itemize}
        \item \texttt{label variable re78 "Real earnings in 1978 (USD)"}
        \vspace{0.2cm}
        % \item \texttt{label define treat\_lbl 0 "Control" 1 "Treated"}
        % \vspace{0.1cm}
        % \item \texttt{label values treat treat\_lbl}
    \end{itemize}
    \vspace{0.2cm}
    \item \textbf{Explore the data} systematically:
    \vspace{0.1cm}
    \begin{itemize}
        \item \texttt{describe}, \texttt{summarize}, \texttt{codebook},
        \texttt{tabulate}, \texttt{inspect}
        \vspace{0.1cm}
        \item Check for \textbf{missing values}, \textbf{outliers},
        and \textbf{implausible values}
    \end{itemize}
    % \vspace{0.2cm}
    % \item AI tools can help: paste your variable list and codebook into
    % ChatGPT or Claude and ask it to \textbf{suggest labels, flag anomalies,
    % or draft cleaning code}
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{CEM: R Example (via \texttt{MatchIt})}{}
\begin{lstlisting}
...
\begin{itemize}
    ...
    covariates listed in the formula are used for matching
    \vspace{0.1cm}
