...
% \begin{frame}{Fuzzy RDD Estimation}{Nonparametric Approach --- Fuzzy Extension}
% 	\begin{itemize}
% 		\item Restrict to observations within bandwidth $h$ of cutoff $c$; weight by kernel $K_h(A_i - c)$
% 		\vspace{0.2cm}
% 		\item Estimate the same \textbf{2SLS} system, now \emph{locally}:
% 		\vspace{0.1cm}
% 		\begin{equation*}
% 			D_i = \alpha_1 + \rho_1 Z_i + \beta_1 (A_i - c) + \upsilon_i \quad \text{(First Stage)}
% 		\end{equation*}
% 		\begin{equation*}
% 			Y_i = \alpha_2 + \rho_2 D_i + \beta_2 (A_i - c) + \eta_i \quad \text{(Second Stage)}
% 		\end{equation*}
% 		\item $\hat{\rho}_2$ is the LATE for compliers at $c$
% 	\end{itemize}
% \end{frame}

% \begin{frame}{Fuzzy RDD Estimation}{Nonparametric Approach --- Key Features}
% 	\begin{itemize}
% 		\item \textbf{Kernel weights}: triangular kernel down-weights observations far from $c$
% 		\begin{equation*}
% 			K_h(u) = \left(1 - \frac{|u|}{h}\right)\cdot\frac{1}{h}, \qquad |u| \leq h
% 		\end{equation*}
% 		\vspace{0.1cm}
% 		\item \textbf{Bandwidth} $h$: chosen to minimise MSE
% 		\vspace{0.1cm}
% 		\begin{itemize}
% 			\item Narrow $h$: low bias, high variance
% 			\item Wide $h$: low variance, high bias from non-linearity in $\mathrm{f}(A_i)$
% 		\end{itemize}
% 		\vspace{0.2cm}
% 		\item \textbf{Inference}: MSE-optimal $h$ introduces bias $\Rightarrow$ standard SE too narrow
% 		\vspace{0.1cm}
% 		\begin{itemize}
% 			\item \citet{Calonico2014}: bias-corrected point estimate + robust confidence interval
% 		\end{itemize}
% 		\vspace{0.2cm}
% 		\item \textbf{Robustness}: confirm $\hat{\rho}_2$ is stable across alternative bandwidths
% 	\end{itemize}
% \end{frame}
...
% \begin{frame}{Fuzzy RDD Estimation}{Nonparametric Approach --- Fuzzy Extension}
% 	\begin{itemize}
% 		\item In \textbf{sharp RDD}: one local linear regression within bandwidth $h$
% 		\begin{equation*}
% 			Y_i = \beta + \alpha D_i + \gamma_1(A_i-c) + \gamma_2 D_i(A_i-c) + \eta_i, \quad |A_i - c| \le h
% 		\end{equation*}
% 		$\hat{\alpha}$ = RD treatment effect (ATE at cutoff)
% 		\vspace{0.2cm}
% 		\item In \textbf{fuzzy RDD}: two local regressions within the \emph{same} bandwidth $h$
% 		\vspace{0.2cm}
% 		\begin{itemize}
% 			\item \textbf{First Stage} (local): regress $D_i$ on $Z_i$ and $(A_i - c)$ within $h$
% 			\vspace{0.2cm}
% 			\item \textbf{Reduced Form} (local): regress $Y_i$ on $Z_i$ and $(A_i - c)$ within $h$
% 			\vspace{0.2cm}
% 			\item LATE $= \hat{\rho}_2^{RF} / \hat{\rho}_1^{FS}$ (Wald ratio at the cutoff)
% 		\end{itemize}
% 		\vspace{0.2cm}
% 		\item \texttt{rdrobust} with \texttt{fuzzy($D_i$)} handles all this automatically:
% 		\vspace{0.2cm}
% 		\begin{itemize}
% 			\item Selects MSE-optimal bandwidth; uses triangular kernel
% 			\vspace{0.2cm}
% 			\item Provides bias-corrected, robust confidence intervals (Calonico et al.\ 2014)
% 		\end{itemize}
% 	\end{itemize}
% \end{frame}
...
\title[]
{Fuzzy RDD: Empirical Example \& Exercise}
...
\begin{frame}{Bleemer \& Mehta (2022)}{STATA Implementation}
	\begin{itemize}
		...
		\vspace{0.2cm}
		\item Use \texttt{simulated\_econ\_rdd.dta} (calibrated to the paper)
		\vspace{0.2cm}
...
\begin{frame}{Step 1: Graphical Analysis}{First Stage}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\vspace{0.2cm}
			\item Inspect whether P(economics major) \textbf{jumps} at GPA = 2.8
			\vspace{0.2cm}
...
\end{lstlisting}
		\item \textbf{R}: use \texttt{rdplot(y = data\$sat\_score, x = data\$gpa\_econ, c = cutoff)}
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Step 3: Preparation for Estimation}{}
	\begin{itemize}
		\item Generate centered running variable and polynomial/interaction terms
\begin{lstlisting}
...
\end{lstlisting}
		...
		\vspace{0.2cm}
		\item \textbf{gpa\_c}: centered running variable (= 0 at cutoff)
		\vspace{0.2cm}
