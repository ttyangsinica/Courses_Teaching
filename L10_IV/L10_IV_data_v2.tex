...
\begin{frame}{Potential Outcomes Framework}{Treatment Assignment}
	\begin{itemize}
...
\begin{frame}{Potential Outcomes Framework}{Potential Treatments}
	\begin{itemize}
...
\begin{frame}{Potential Outcomes Framework}{Potential Outcomes}
	...
	\begin{itemize}
		...
		\vspace{0.2cm}
		\begin{itemize}
			\item $\mathrm{Y}^{1}_{i}$: earnings if individual $i$ serves in the military
			\vspace{0.2cm}
			\item $\mathrm{Y}^{0}_{i}$: earnings if individual $i$ does not serve in the military
			\vspace{0.2cm}
		\end{itemize}
		\vspace{0.2cm}
		\item \textcolor{blue}{Observed Outcome}
		\begin{equation*}
			Y_{i}=D_{i}\mathrm{Y}^{1}_{i}+(1-D_{i})\mathrm{Y}^{0}_{i}
		\end{equation*}
		\vspace{0.2cm}
		\item This notation already imposes the exclusion idea: once treatment status is fixed, $Z_{i}$ has no separate effect on $Y_{i}$
		
	\end{itemize}
\end{frame}


\begin{frame}{Identification Roadmap}
	\begin{itemize}
		\item We observe how outcomes and treatment rates differ by draft eligibility:
		\begin{equation*}
			\alpha_{IV}=
			\dfrac{\mathrm{E}[Y_{i}|Z_{i}=1]-\mathrm{E}[Y_{i}|Z_{i}=0]}
			{\mathrm{E}[D_{i}|Z_{i}=1]-\mathrm{E}[D_{i}|Z_{i}=0]}
		\end{equation*}
		\vspace{0.2cm}
		\item To interpret this ratio causally, we need to answer three questions:
		\vspace{0.2cm}
		\begin{itemize}
			\item Why does comparing $Z_i=1$ and $Z_i=0$ recover causal variation?
			\vspace{0.2cm}
			\item Through which channel can $Z_i$ affect earnings?
			\vspace{0.2cm}
			\item Whose treatment status is actually changed by $Z_i$?
		\end{itemize}
	\end{itemize}
\end{frame}


\begin{frame}{Identification Results for IV}{Observed Wald Ratio}
	\begin{figure}
	\begin{tikzpicture}[comment/.style={rectangle, text centered}]
		\centering
...
\end{figure}
	...
	\begin{itemize}
		\item IV uses the causal chain from draft eligibility $Z_{i}$ to military service $D_{i}$ to earnings $Y_{i}$
		\vspace{0.2cm}
		\item Intuitively:
		\begin{equation*}
		\begin{aligned}
		& \text{Effect of } Z_i \text{ on } Y_i \\ 
		&=	(\text{Effect of } Z_i \text{ on } D_i)
		\times (\text{Effect of } D_i \text{ on } Y_i)
		\end{aligned}
		\end{equation*}
...
\begin{frame}{Identification Results for IV}{Observed Wald Ratio}
	\begin{itemize}
		...
		\item Rearranging gives the Wald estimand:
		\begin{equation*}
			\alpha_{IV}=\dfrac{\mathrm{E}[Y_{i}|Z_{i}=1]-\mathrm{E}[Y_{i}|Z_{i}=0]}{\mathrm{E}[D_{i}|Z_{i}=1]-\mathrm{E}[D_{i}|Z_{i}=0]}
		\end{equation*} 
		
		\item This ratio has two observable pieces:
				\vspace{0.2cm}
		\begin{itemize}
			\item Reduced-form effect: the numerator captures how draft eligibility changes average earnings
					\vspace{0.2cm}
			\item First-stage effect: the denominator captures how draft eligibility changes the probability of military service
					\vspace{0.2cm}
		...
		\item The remaining question: what causal effect does this ratio identify?
...
\begin{frame}{Identification Assumptions for IV}{First-Stage Relationship}
	\begin{itemize}
		...
		\item \textbf{First-Stage Relationship}: $Z_{i}$ affects treatment status $D_{i}$
...
\begin{frame}{Identification Assumptions for IV}{Independence}
	\begin{itemize}
	
		\item \textbf{Independence Assumption}: $Z_{i}$ is independent of potential outcomes and potential treatments
		...
		\vspace{0.2cm}
				\begin{itemize}
		\item Draft eligibility is as good as randomly assigned
		\vspace{0.2cm}
		\item Therefore, potential earnings and potential treatment choices are balanced across $Z_i=1$ and $Z_i=0$
		\vspace{0.2cm}
		\item This lets us replace conditional expectations given $Z_i$ with unconditional expectations in the proof
				\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Identification Assumptions for IV}{Exclusion Restriction}
	\begin{itemize}
		...
		\vspace{0.2cm}
		\begin{itemize}	
			\item The instrument has no direct effect on the outcome once treatment status is fixed
			\vspace{0.2cm}
			\item In the draft lottery example, draft eligibility should affect earnings only by changing military service
		
			\begin{equation*}
				Y_i = D_i \mathrm{Y}^{1}_{i}+(1-D_i)\mathrm{Y}^{0}_{i}
			\end{equation*}
...
\begin{frame}{Identification Assumptions for IV}{Monotonicity Assumption}
	\begin{itemize}
		...
		\vspace{0.2cm}
		\item In the draft lottery example, eligibility should weakly encourage military service
			\end{itemize}
...
\begin{frame}{IV and Compliance Types}
	\begin{itemize}
		\item Because draft eligibility does not perfectly determine military service, people can react differently to $Z_i$
		\vspace{0.2cm}
		\item Each individual has two potential treatment statuses:
		\vspace{0.2cm}
		\begin{itemize}
			\item $\mathrm{D}^{1}_{i}$: treatment status if draft eligible
			\vspace{0.2cm}
			\item $\mathrm{D}^{0}_{i}$: treatment status if not draft eligible
		\end{itemize}
		\vspace{0.2cm}
		\item The pair $(\mathrm{D}^{1}_{i},\mathrm{D}^{0}_{i})$ tells us whose behavior the instrument can change
...
% 		\item Four types of individuals:
% 		\vspace{0.2cm}
% 		\begin{itemize}
% 			\vspace{0.3cm}
% 			\item \textbf{Compliers}: $\mathrm{D}^{1}_{i}=1$ and $\mathrm{D}^{0}_{i}=0$
% 			\begin{itemize}
% 				\item Serve only when draft eligible
% 			\end{itemize}
% 			\item \textbf{Always-takers}: $\mathrm{D}^{1}_{i}=\mathrm{D}^{0}_{i}=1$
% 			\begin{itemize}
% 				\item Serve regardless of draft eligibility
% 				\vspace{0.2cm}
% 			\end{itemize}			
% 			\item \textbf{Never-takers}: $\mathrm{D}^{1}_{i}=\mathrm{D}^{0}_{i}=0$
% 			\begin{itemize}
% 				\item Do not serve regardless of draft eligibility
% 				\vspace{0.2cm}
% 			\end{itemize}			
% 			\item \textbf{Defiers}: $\mathrm{D}^{1}_{i}=0$ and $\mathrm{D}^{0}_{i}=1$
% 			\begin{itemize}
% 				\item Serve only when not draft eligible
% 			\end{itemize}
...
\begin{frame}{Identification Results for IV}{What the Wald Ratio Identifies}
	\begin{itemize}
		...
		\item IV identifies the average causal effect for individuals whose treatment status is changed by the instrument
		\vspace{0.2cm}
		\item In the draft lottery example, this is the effect of military service for men induced to serve because they became draft eligible
	\end{itemize}
\end{frame}



\begin{frame}{Identification Results for IV}{Proof: Convert Observed Differences}
	\textcolor{blue}{Proof:}
	\begin{equation*}
		\begin{aligned}
			&\alpha_{IV}=\dfrac{\mathrm{E}[Y_{i}|Z_{i}=1]-\mathrm{E}[Y_{i}|Z_{i}=0]}{\mathrm{E}[D_{i}|Z_{i}=1]-\mathrm{E}[D_{i}|Z_{i}=0]}  \\
...
\begin{frame}{Identification Results for IV}{Proof: Use Compliance Types}
	\begin{itemize}
		
		\item Since $\mathrm{D}^{z}_{i}$ is a dummy, $\mathrm{D}^{1}_{i}-\mathrm{D}^{0}_{i}$ can only be $1$, $0$, or $-1$
		\vspace{0.2cm}
		\begin{itemize}
			\item \textbf{Always-takers} and \textbf{never-takers}: $\mathrm{D}^{1}_{i}-\mathrm{D}^{0}_{i}=0$
			\vspace{0.2cm}
			\item \textbf{Compliers}: $\mathrm{D}^{1}_{i}-\mathrm{D}^{0}_{i}=1$
			\vspace{0.2cm}
			\item \textbf{Defiers}: $\mathrm{D}^{1}_{i}-\mathrm{D}^{0}_{i}=-1$	
			\vspace{0.2cm}	
		\end{itemize}	
		\item Under monotonicity, defiers are ruled out:
		\begin{equation*}
			\begin{aligned}
				& \mathrm{E}[(\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i})(\mathrm{D}^{1}_{i}-\mathrm{D}^{0}_{i})]  \\
				& = \mathrm{E}[(\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}) \times (1)|\mathrm{D}^{1}_{i}-\mathrm{D}^{0}_{i}=1] \Pr(\mathrm{D}^{1}_{i}-\mathrm{D}^{0}_{i}=1) \\
				& = \mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|\mathrm{D}^{1}_{i}>\mathrm{D}^{0}_{i}] \Pr(\mathrm{D}^{1}_{i}-\mathrm{D}^{0}_{i}=1)
			\end{aligned}
		\end{equation*}
...
\begin{frame}{Identification Results for IV}{Proof: Interpret the First Stage}
	\begin{itemize}
...
%		\end{itemize}
%		\vspace{0.2cm}
		\item The denominator is the first-stage effect:
		\begin{equation*}
			\begin{aligned}
		...
		\end{equation*}
		\vspace{0.2cm}
		\item Under monotonicity, $\Pr(\mathrm{D}^{0}_{i}=1, \mathrm{D}^{1}_{i}=0) = 0$
		\vspace{0.2cm}
		\item Therefore, the first stage is the share of compliers:
		\begin{equation*}
...
\begin{frame}{Identification Results for IV}{Proof: LATE}
	\begin{itemize}
		\item Combine the numerator and denominator results
	\end{itemize}			
	\vspace{0.2cm}
	\textcolor{blue}{Proof:}
	\begin{equation*}
...
\begin{frame}{Identification Results for IV}{Why Local?}
	
	\begin{itemize}
		\vspace{0.2cm}
		\item \textbf{Never-takers} and \textbf{always-takers} do not change treatment status when the instrument changes
		\vspace{0.2cm}
		\begin{itemize}
			\item They do not contribute to the first stage
			\vspace{0.2cm}
			\item Their treatment effects are not revealed by this instrument
			\vspace{0.2cm}
		\end{itemize}
		\item Monotonicity rules out \textbf{defiers}
		\vspace{0.2cm}
		\item Therefore, IV identifies the average treatment effect for \textbf{compliers}
	\end{itemize}
\end{frame}
...
\begin{frame}{Identification Results for IV}{LATE}
	\textcolor{blue}{LATE}\\
	$\alpha_{\text{LATE}}=E[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|\mathrm{D}^{1}_{i}>\mathrm{D}^{0}_{i}]$, the average treatment effect for compliers, is called the \textbf{Local Average Treatment Effect (LATE)}.
	\begin{itemize}
		\vspace{0.2cm}
		\item It is local because it applies to the subpopulation whose treatment status is changed by the instrument
		\vspace{0.2cm}
			\begin{itemize}
		\item In this example: men induced into military service by draft eligibility
		\vspace{0.2cm}
			\end{itemize}
		%\begin{itemize}
		...
		%\vspace{0.2cm}
		%\end{itemize}
		\item Different instruments may move different groups of compliers
		\vspace{0.2cm}
					\begin{itemize}
		\item Whether LATE is interesting depends on whether those compliers are substantively important
					\end{itemize}
	\end{itemize}
\end{frame}
