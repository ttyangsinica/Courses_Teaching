...
\begin{equation*}
\alpha_{\text{ATT}} = \mathrm{E}[\tau_i|D_i=1]= \mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|D_i=1] = \dfrac{1}{\color{red}N_1} \sum_{\color{red}i:D_i=1}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}]
\end{equation*}
%where $N_1 = \sum_i \oldstylenums{1}(D_i=1)$
%\vspace{0.2cm}
\begin{itemize}
...
\vspace{0.2cm}
\begin{itemize} 
	\item Average effect of attending graduate school on annual earnings for those attending graduate school ($D_{i}=1$)
\vspace{0.2cm}	
\item Average difference between the earnings of those attending graduate schools v.s. earnings if they had not attended graduate schools 
\end{itemize}
%\vspace{0.2cm}
%\item We'll also spend a lot time trying to identify/estimate ATT
...
\begin{frame}{Causal Effect for a Control Group}
	\begin{itemize}	
		...
		\begin{equation*}
			\alpha_{\text{ATU}} = \mathrm{E}[\tau_i|D_i=0]= \mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|D_i=0] = \dfrac{1}{\color{red}N_0} \sum_{\color{red}i:D_i=0}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}]
		\end{equation*}
	%	where $N_0 = \sum_i \oldstylenums{1}(D_i=0)$
	%	\vspace{0.2cm}
		\begin{itemize}
	...
	\item Average difference in potential outcomes for those who were untreated
			...
			\begin{itemize} 
				\item Average effect of attending graduate school on annual earnings for those NOT attending graduate school ($D_{i}=0$)	
							\vspace{0.2cm}
				\item Average difference between the earnings of those NOT attending graduate schools v.s. earnings if they had attended graduate schools 
			\end{itemize}
			%\vspace{0.2cm}
			%\item We'll also spend a lot time trying to identify/estimate ATT
