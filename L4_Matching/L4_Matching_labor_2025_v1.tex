...
\begin{frame}{Conditional Independence Assumption}{Example}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\vspace{0.2cm}
					\begin{itemize}
			\item Let us estimate the \textbf{counterfactual} of what trainees would have earned without training by using observed earnings of non-trainees
						\vspace{0.2cm}
			\end{itemize}
			\item $\textcolor{red}{\mathrm{E}[\mathrm{Y}^{\textbf{1}}_{i}|X_{i}=40, D_i=0]}=\mathrm{E}[\mathrm{Y}^{\textbf{1}}_{i}|X_{i}=40, D_i=1]$
			\vspace{0.2cm}
			\begin{itemize}
				\item Let us estimate the \textbf{counterfactual} of what non-trainees would have earned with training by using observed earnings of trainees
			\end{itemize}
		\end{itemize}
		
		%\item Within the 40-year-old group, the average potential earnings for trained individuals would have been the same as for untrained individuals had they not received training
...
\begin{frame}{Common Support Assumption}
	\begin{itemize}
		...
		\end{itemize}
				\vspace{0.2cm}
		\item It ensures that there is sufficient overlap in the characteristics of treated and untreated units to find adequate matched sample	
...
\begin{frame}{Identification Results for Matching}
	...
	\end{equation*}	
		\begin{itemize}	
		\item Remember CIA ensures $\mathrm{E}[\mathrm{Y}^{0}_{i}|X_{i},D_i=1] = \mathrm{E}[\mathrm{Y}^{0}_{i}|X_{i},D_i=0]$
...
\begin{frame}{Identification Results for Matching}
	\begin{itemize}
		\item Under CIA, matching estimator can identify CATT, CATU or CATE
		\vspace{0.2cm}
		\begin{itemize}
			\item Causal effect for the people with specific value of $X_{i}$
			\vspace{0.2cm}
					\begin{itemize}
			\item Causal effect of job training on annual earnings for people with age 40 (CATE)
				\vspace{0.2cm}
			\item Causal effect of job training on annual earnings for treated people with age 40 (CATT)
					\end{itemize}
		\end{itemize}
			\vspace{0.2cm}
		\item How to obtain ATT, ATU, and ATE?
			\vspace{0.2cm}
		\begin{itemize}
			\item Take average of CATT, CATU, or CATE over all subgroups (all possible X-values)
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Review: The Law of Iterated Expectations (LIE)}
	...
	\begin{itemize}
		\item Intuitively, there are two ways to compute average outcome $Y$ (e.g. earnings)
		\vspace{0.2cm}
		\item[1] $\mathrm{E}[\mathrm{Y}_{i}] = \sum_{\mathrm{Y}^{}_{i}} \mathrm{Y}_{i} \times Pr(\mathrm{Y}_{i}=y)$
		\vspace{0.2cm}
			\begin{itemize}
		\item Each value of outcome times its probability
					\vspace{0.2cm}
			\end{itemize}
		\item[2] $\mathrm{E}[\mathrm{E}[\mathrm{Y}_{i} | \mathrm{X}_{i}]] = \sum_{\mathrm{X}^{}_{i}} \mathrm{E}[\mathrm{Y}_{i} | \mathrm{X}_{i}] \times Pr(\mathrm{X}_{i}=x) = \mathrm{E}[\mathrm{Y}_{i}]$ 
			\begin{itemize}
							\vspace{0.2cm}
			\item Average outcome of subgroup times the share of subgroup
		\end{itemize}
...
\begin{frame}{Review: The Law of Iterated Expectations (LIE)}{Example}
	
	
	\begin{itemize}
		
		\item Suppose we want to compute average income in Taiwan
		\vspace{0.2cm}
		\item[1] We know 40\% of people in Taiwan earn 1 million NTD per year, 40\% earn 2 million NTD, and 20\% earn 3 million NTD  
		\vspace{0.2cm}
		
		\begin{equation*}
		\begin{aligned}
		\mathrm{E}[\mathrm{Y}_{i}] &= \sum_{\mathrm{Y}^{}_{i}} \mathrm{Y}_{i} \times Pr(\mathrm{Y}_{i}=y) \\
		&= 1 \times 0.4 + 2 \times 0.4 + 3 \times 0.2 \\ 
		&= 1.8
		\end{aligned}
		\end{equation*}	
...
\begin{frame}{Review: The Law of Iterated Expectations (LIE)}{Example}
	...
	\begin{itemize}
		...
		\item[2] Suppose we know the average annual income for male is 2 million and average annual income for female is 1.6 million
		\vspace{0.2cm}
		\item The share of male to total population is 50\%
		\vspace{0.2cm}
		\begin{equation*}
		\begin{aligned}
		\mathrm{E}[\mathrm{E}[\mathrm{Y}_{i} | \mathrm{X}_{i}]] &= \sum_{\mathrm{X}^{}_{i}} \mathrm{E}[\mathrm{Y}_{i} | \mathrm{X}_{i}] \times Pr(\mathrm{X}_{i}=x) \\
		&= 2 \times 0.5 + 1.6 \times 0.5  \\
		&= 1.8  \\
		&= \mathrm{E}[\mathrm{Y}_{i}] 
		\end{aligned}
		\end{equation*}	
...
\begin{frame}{Identification Results for Matching: ATT}
	\begin{itemize}
		
		\item Using a matching method, we can also identify CATT
		\vspace{0.2cm}
		\begin{equation*}
			\begin{aligned}
		...
		\item Applying LIE, we can identify ATT by averaging all of the $X$-specific effects in treatment group (CATT):
		\vspace{0.2cm}
		
	\end{itemize}
	\begin{equation*}
		\begin{aligned}
			\mathrm{E}[\underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|X_{i},D_i=1 |D_i=1]}_{\text{CATT}}] 
...
\begin{frame}{Identification Results for Matching: ATT}{A Numerical Example}
	\begin{itemize}
			...
			\item Using matching methods, we get CATT for all possible ages 
			
			\vspace{0.2cm}
			\begin{equation*}
				\begin{aligned}
					& \mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|X_{i}=28, D_{i}=1] = \underbrace{\mathrm{E}[\mathrm{Y}_{i}|X_{i},D_{i}=1] - \mathrm{E}[\mathrm{Y}_{i}|X_{i},D_{i}=0]}_{\text{ODO at given $X_{i}=28$}} \\ 
					&= \dfrac{(17700-8800)+(11500-8800)+ (16300-8800)}{3} \\
					&= 6366.67 \\
					& \vdots \\
					& \mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|X_{i}=34, D_{i}=1] = \underbrace{\mathrm{E}[\mathrm{Y}_{i}|X_{i},D_{i}=1] - \mathrm{E}[\mathrm{Y}_{i}|X_{i},D_{i}=0]}_{\text{ODO at given $X_{i}=34$}} \\ 
					&= \dfrac{10200-24200}{1}  \\
					&= −14,000
...
\begin{frame}{Identification Results for Matching: ATT}{A Numerical Example}
	...
	\item Applying LIE, we can identify ATT by averaging CATT over all possible ages:
	\vspace{0.2cm}
	
	
	\begin{equation*}
		\begin{aligned}
			& \mathrm{E}[\underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|X_{i}, D_{i}=1]}_{\text{CATT}}] \\
			&= \mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|X_{i}=28, D_{i}=1] \times \frac{3}{19}  \\ 
			&  + \mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|X_{i}=34, D_{i}=1] \times \frac{1}{19} + \cdots \\
...
\begin{frame}{Identification Results for Matching: ATU}
	\begin{itemize}
		
		\item Note that using a matching method, we can also identify CATU
		\vspace{0.2cm}
		\begin{equation*}
			\begin{aligned}
	\underbrace{\mathrm{E}[\mathrm{Y}_{i}|X_{i},D_{i}=1] - \mathrm{E}[\mathrm{Y}_{i}|X_{i},D_{i}=0]}_{\text{ODO at given $X_{i}$}}  &= \underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|X_{i},D_i=0]}_{\text{CATU}}
			\end{aligned}
		\end{equation*}	
		
		
		\item Applying LIE, we can identify ATU by averaging all of the $X$-specific effects in treatment group (CATU):
		\vspace{0.2cm}
		
	\end{itemize}
	\begin{equation*}
		\begin{aligned}
			\mathrm{E}[\underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|X_{i},D_i=0 |D_i=0]}_{\text{CATU}}] 
...
\begin{frame}{Identification Results for Matching: ATE}
	\begin{itemize}
		
		\item Using a matching method, we can identify CATE
		\vspace{0.2cm}
		\begin{equation*}
			\begin{aligned}
				\underbrace{\mathrm{E}[\mathrm{Y}_{i}|X_{i},D_{i}=1] - \mathrm{E}[\mathrm{Y}_{i}|X_{i},D_{i}=0]}_{\text{ODO at given $X_{i}$}}  &= \underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|X_{i}]}_{\text{CATE}} 
			\end{aligned}
		\end{equation*}	
		
		
		\item Applying LIE, we can identify ATE by averaging all of the $X$-specific effects (CATE):
		\vspace{0.2cm}
		
	\end{itemize}
	\begin{equation*}
		\begin{aligned}
			\mathrm{E}[\underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|X_{i}]}_{\text{CATE}}] 
...
%\item Again, if we have population data, we can get the above causal effect	
%		\vspace{0.2cm}
...
\begin{frame}{Distance Matching}{Criteria for a Good Match}
	...
	\begin{itemize}
		...
		%\vspace{0.2cm}
		%\item If all control units have equal weight, we call this radius matching
		%\vspace{0.2cm}
		%\item If weights decay with distance, we call this kernel matching
	\end{itemize}
...
\vspace{0.2cm}
%\item Matching discrepancies converge to zero 
%\vspace{0.2cm}
%\item But they converge very slowly if $k$ is large
%\vspace{0.2cm}
%\item Mathematically, it can be shown that $||X_i-X_{j(i)}||$ converges to zero at the same rate as $\dfrac{1}{N^{1/k}}$
%\vspace{0.2cm}
...
%\item PSM methods can be adapted to multivalued treatment, through this has yet to catch on: see Imbens (2000)
\end{itemize}
\end{itemize}
\end{frame}
...
%to have independence between the treatment status and the potential outcomes
\vspace{0.2cm}
...
\begin{frame}{Identification Results or PSM}
	\begin{itemize}	
		
		\item Similar to identification results of matching estimator, the only difference is that we match treated and untreated units based on $p(X_{i})$ instead of a set of covariates $X_{i}$
			\vspace{0.2cm}
		\item Remember CIA and propensity score theorem ensures:
					\vspace{0.2cm}
			\begin{itemize}	
			\item  $\textcolor{red}{\mathrm{E}[\mathrm{Y}^{0}_{i}|p(X_{i}),D_i=1]} = \mathrm{E}[\mathrm{Y}^{0}_{i}|p(X_{i}),D_i=0]$
			\vspace{0.2cm}
			\item  $\textcolor{red}{\mathrm{E}[\mathrm{Y}^{1}_{i}|p(X_{i}),D_i=0]} = \mathrm{E}[\mathrm{Y}^{1}_{i}|p(X_{i}),D_i=1]$
		\end{itemize}
...
& 
	...
	\\
	& + {\color{red} \mathrm{E}[\mathrm{Y}^{0}_{i}|p(X_{i}),D_i=1]}-\mathrm{E}[\mathrm{Y}^{0}_{i}|p(X_{i}),D_i=0] \\
	&= \underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|p(X_{i}),D_i=1]}_{\text{CATT}} \\
	&+ \underbrace{\mathrm{E}[\mathrm{Y}^{0}_{i}|p(X_{i}),D_i=1]-\mathrm{E}[\mathrm{Y}^{0}_{i}|p(X_{i}),D_i=0]}_{\text{Selection Bias}} \\
	&= \underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|p(X_{i}),D_i=1]}_{\text{CATT}} + \underbrace{0}_{\text{Selection Bias}} \\
		&= \underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|p(X_{i}),D_i=0]}_{\text{CATU}}= \underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|p(X_{i})]}_{\text{CATE}}
...
\begin{frame}{Identification Results or PSM}{ATT}
	\begin{itemize}
		
		\item Note that using a matching method, we can also identify CATT
		\vspace{0.2cm}
		\begin{equation*}
			\begin{aligned}
				\underbrace{\mathrm{E}[\mathrm{Y}_{i}|p(X_{i}),D_{i}=1] - \mathrm{E}[\mathrm{Y}_{i}|p(X_{i}),D_{i}=0]}_{\text{ODO at given $p(X_{i})$}} &= \underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|p(X_{i}),D_i=1]}_{\text{CATT}}
			\end{aligned}
		\end{equation*}	
		
		
		\item Applying LIE, we can identify ATT by averaging all of the $p(X)$-specific effects for treatment group (CATT):
		\vspace{0.2cm}
		
	\end{itemize}
	\begin{equation*}
		\begin{aligned}
			\mathrm{E}[\underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|p(X_{i}),D_i=1 |D_i=1]}_{\text{CATT}}] 
			=\underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|D_i=1]}_{\text{ATT}}
...
\begin{frame}{Identification Results or PSM}{ATU}
	\begin{itemize}
		...
		\begin{equation*}
			\begin{aligned}
				\underbrace{\mathrm{E}[\mathrm{Y}_{i}|p(X_{i}),D_{i}=1] - \mathrm{E}[\mathrm{Y}_{i}|p(X_{i}),D_{i}=0]}_{\text{ODO at given $p(X_{i})$}} &= \underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|p(X_{i}),D_i=0]}_{\text{CATU}}
			\end{aligned}
		\end{equation*}	
		
		
		\item Applying LIE, we can identify ATU by averaging all of the $p(X)$-specific effects for treatment group (CATU):
		\vspace{0.2cm}
		
	\end{itemize}
	\begin{equation*}
		\begin{aligned}
			\mathrm{E}[\underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|p(X_{i}),D_i=0 |D_i=0]}_{\text{CATU}}] 
			=\underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|D_i=0]}_{\text{ATU}}
...
\begin{frame}{Identification Results or PSM}{ATE}
	\begin{itemize}
		...
		\begin{equation*}
		\begin{aligned}
	\underbrace{\mathrm{E}[\mathrm{Y}_{i}|p(X_{i}),D_{i}=1] - \mathrm{E}[\mathrm{Y}_{i}|p(X_{i}),D_{i}=0]}_{\text{ODO at given $p(X_{i})$}} &= \underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|p(X_{i})]}_{\text{CATE}} 
		\end{aligned}
		\end{equation*}	
		
		
		\item Applying LIE, we can identify ATE by averaging all of the $p(X)$-specific effects (CATE):
		\vspace{0.2cm}
		
	\end{itemize}
	\begin{equation*}
	\begin{aligned}
	\mathrm{E}[\underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|p(X_{i})]}_{\text{CATE}}] 
	=\underbrace{\mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}]}_{\text{ATE}}
...
\begin{frame}{Propensity Score Matching}{Estimation}
	\begin{itemize}
		...
		\end{itemize}
			...
			\end{itemize}
			%\vspace{0.2cm}
		%\item[2] Skip the cumbersome matching procedure and use weighting approach
...
\begin{frame}{STATA Example}{Dehejia et al. (1999)}	Rajeev H. Dehejia; Sadek Wahba (1999) ``\textbf{Causal Effects in Nonexperimental Studies: Reevaluating the Evaluation of Training Programs}" Journal of the American Statistical Association 
	\begin{itemize}
		\vspace{0.2cm}
		%\item Use an empirical example in this paper
		%\vspace{0.2cm}
...
\begin{itemize}
	...
	%	\vspace{0.2cm}
	%	\item Useful for assessing baseline balance in outcomes before causal analysis
	\end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	%	\vspace{0.2cm}
	%	\item Helps determine if treatment and control groups are comparable before causal analysis
	\end{itemize}
\end{itemize}
\end{frame}
...
\end{lstlisting}
	\begin{itemize}
		...
		\vspace{0.2cm}
			\begin{itemize}
		%\item \textbf{gen(stub)}: specifies that the observation numbers of the nearest neighbors be stored in the new variables stub1, stub2, ....
		%\vspace{0.2cm}
...
\begin{frame}[fragile]{STATA Example}{Step 3: PSM Estimation -- teffects psmatch}
	\begin{itemize}
		\begin{figure}[p]
			\centering
			\includegraphics[width=1.0\textwidth]{tteffects_f1_2.jpg}
		\end{figure}
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{STATA Example}{Step 3: PSM Estimation -- teffects psmatch}
	\item Understanding the matching process:
\begin{lstlisting}
...
\begin{frame}[fragile]{Path Setup}{}
	%\item Environment and Path Setup:
\begin{lstlisting}
rm(list = ls())
...
\end{lstlisting}
	\begin{itemize}
		...
		%	\vspace{0.2cm}
		%	\item \texttt{Matching} provides functions for propensity score matching
		\end{itemize}
...
\begin{lstlisting}
lalonde_csv <- fread(paste0(rawdata, "/lalonde.csv"), data.table = FALSE) # Return as data.frame
...
write_dta(lalonde_csv, paste0(rawdata, "/lalonde.dta"), version = 14) # Specify Stata version compatibility
\end{lstlisting}
...
\begin{itemize}
	\vspace{0.2cm}
	\item Display frequency and percentage tables for the \texttt{race} variable
	\vspace{0.2cm}
	\begin{itemize}
		\item \textbf{table():} Shows the count for each race category in the dataset
		\vspace{0.2cm}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		%\vspace{0.2cm}
		%\item Useful for indexing observations, merging datasets, or tracking data transformations
	\end{itemize}
	\vspace{0.2cm}
...
\end{lstlisting}
	\begin{itemize}
		...
		%\vspace{0.2cm}
		%\item \textbf{estimand="ATT":} focuses on treatment effect for treated units
		%\vspace{0.2cm}
