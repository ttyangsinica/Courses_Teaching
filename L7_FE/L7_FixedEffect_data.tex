...
\begin{frame}{Fixed Effects Regression}
	\begin{itemize}
		...
		\vspace{0.2cm}
		\begin{itemize}
			\item With panel data (multiple observations per unit over time), we can control for \textit{all} time-invariant unobservables — even ones we cannot measure
			\vspace{0.2cm}
			\item The key idea: assign each individual (firm, state, \ldots) their own intercept $\lambda_i$ that absorbs every time-invariant characteristic of unit $i$
			% \vspace{0.2cm}
			% \item We then identify $\alpha$ purely from \textbf{within-unit changes} over time
		\end{itemize}

		\vspace{0.2cm}
		\item \textbf{Why ``fixed effects''?}
		\vspace{0.2cm}
		\begin{itemize}
			\item $\lambda_i$ is treated as a \textbf{fixed (non-random) parameter} to be estimated — one for every unit $i$
			\vspace{0.2cm}
			\item This contrasts with \textbf{random effects}, where $\lambda_i \sim N(0,\sigma^2_\lambda)$ is assumed to be a random draw \textit{uncorrelated} with $D_{it}$
			\vspace{0.2cm}
			\item Because FE makes no distributional assumption on $\lambda_i$, it is consistent even when $\lambda_i$ is correlated with the treatment
		\end{itemize}
...
\begin{frame}{Fixed Effects Regression}{Example}
	\begin{itemize}
		...
		\item Key assumption: $E[\varepsilon_{it}\mid A_i, D_{it}, X_{it}]=0$
		\vspace{0.2cm}
		\begin{itemize}
			\item After controlling for $A_i$ and $X_{it}$, the treatment $D_{it}$ is as good as randomly assigned
			% \vspace{0.2cm}
			% \item This rules out reverse causality where past outcomes affect current treatment (\textit{feedback effects})

            %Known as \textbf{strict exogeneity}:
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Demeaning Approach}{}
	\begin{itemize}
		...
		\item $\lambda_i$ drops out because it is time-invariant ($\bar{\lambda}_i = \lambda_i$)
		\vspace{0.2cm}
		\item \textbf{Intuition}: identification comes entirely from \textbf{within-unit variation} 
		\vspace{0.2cm}
		\begin{itemize}
        \item How a worker's union status and earnings change \textit{over time}, not from comparing different workers cross-sectionally
        \vspace{0.2cm}
			\item This is why the demeaned estimator is also called the \textbf{within estimator}
			\vspace{0.2cm}
			\item Cross-sectional differences between individuals (including $A_i$) are completely removed
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Regression with `N-1 dummy variables"}
	\begin{itemize}
		...
		\vspace{0.2cm}
		\item When there are not many groups (e.g. state, year), we usually control fixed effects by simply including dummy variables
		% \vspace{0.3cm}
		% \item \textbf{Cost of fixed effects}: every unit $i$ consumes one degree of freedom
		% \vspace{0.2cm}
		% \begin{itemize}
		% 	\item With many units and few time periods (large $N$, small $T$), we lose many degrees of freedom
		% 	\vspace{0.2cm}
		% 	\item \textbf{Incidental parameters problem}: with fixed $T$, the estimated $\hat{\lambda}_i$ are inconsistent as $N \to \infty$; in nonlinear models (logit, probit) this can bias the structural parameter $\alpha$ — in linear models OLS remains consistent
		% \end{itemize}
	\end{itemize}
\end{frame}
