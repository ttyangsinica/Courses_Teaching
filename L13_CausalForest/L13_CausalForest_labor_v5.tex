...
\begin{itemize}
  ...
  \]
  \vspace{0.2cm}
  \item But treatment effects may \textbf{vary across individuals} 
  \vspace{0.2cm}
\begin{itemize}

  \item A job training program may help young low-education workers more than older workers
 \end{itemize}
...
\begin{tabular}{lccc}
\toprule
Worker type & Training earnings & Control earnings & Effect \\
\midrule
\multicolumn{4}{l}{\textit{Age $\times$ Education}} \\
\quad Young, low education  & \$11,000 & \$8,000  & \$3,000 \\
\quad Young, high education & \$15,000 & \$14,000 & \$1,000 \\
\quad Older, low education  & \$10,500 & \$8,500  & \$2,000 \\
\quad Older, high education & \$22,000 & \$21,500 & \$500   \\
\midrule
Average treatment effect     & \$14,625 & \$13,000 & \$1,625 \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
  \item Two common ways to explore heterogeneity:
  \vspace{0.2cm}
  \begin{itemize}
    \item[1.] \textbf{Subgroup analysis}: split the sample by gender, age, or education and compare ATE across groups
    \vspace{0.15cm}
    \item[2.] \textbf{Interaction terms}:
    \[
      Y_i = \alpha + \beta D_i + \gamma X_i + \delta\,(D_i \times X_i) + \varepsilon_i
    \]
    $\delta$ tells us how the treatment effect changes with $X_i$
  \end{itemize}
  \vspace{0.25cm}
  \item \textbf{Limitation}: you must decide in advance which $X$ matters --- and the interaction is assumed to be linear
\end{itemize}
\end{frame}


\begin{frame}{Treatment Effect Heterogeneity}{A Data-Driven Approach}
\begin{itemize}
  \item Goal: estimate $\tau(x)$, the treatment effect for a worker \emph{with characteristics $x$}
  \vspace{0.25cm}
  \item Idea: let the data tell us which characteristics matter --- no need to pre-specify
  \vspace{0.25cm}
  \item We will use \textbf{Causal Forest}, a machine-learning method that does this automatically:
\end{itemize}
\vspace{0.2cm}
\begin{center}
\begin{tikzpicture}[
  ...
  \node[box, fill=Green!8,  right=of data]  (trees) {Many causal\\trees};
...
\vspace{0.15cm}
\begin{itemize}
  \item We will explain the details shortly --- for now, think of it as an automatic subgroup finder
\end{itemize}
\end{frame}
...
% --- Slide 1: tree diagram (visual only) ---
\begin{frame}{Decision Trees}{Finding the Best Split}

\begin{center}
{\normalsize Goal: find the variable $X^j$ and split point $s$ that best separates the data}

\vspace{0.5cm}

\begin{tikzpicture}[
  level 1/.style={sibling distance=5.2cm, level distance=2.2cm},
  level 2/.style={sibling distance=2.8cm, level distance=2.0cm},
...
\node[internal] {age $<30$?}
  child {node[leaf] {$R_1$\\[3pt] $\bar{Y}_{R_1}=\$8{,}200$}
    edge from parent node[left, draw=none, fill=none, font=\small] {Yes}
  }
  child {node[internal] {educ $<12$?}
    child {node[leaf] {$R_2$\\[3pt] $\bar{Y}_{R_2}=\$10{,}100$}
      edge from parent node[left, draw=none, fill=none, font=\small] {Yes}
    }
    child {node[leaf] {$R_3$\\[3pt] $\bar{Y}_{R_3}=\$11{,}500$}
      edge from parent node[right, draw=none, fill=none, font=\small] {No}
    }
    edge from parent node[right, draw=none, fill=none, font=\small] {No}
  };
\end{tikzpicture}
...
\vspace{0.5cm}
{\small Each leaf predicts the \textbf{mean} $Y$ of observations that fall in that region}
\end{center}

\end{frame}


% --- Slide 2: algorithm / formula ---
\begin{frame}{Decision Trees}{Finding the Best Split}
\textbf{How to find $(X^j,\, s)$ that minimises prediction error:}
\vspace{0.2cm}
\begin{enumerate}
  \item For each variable $X^j$ and candidate split point $s$: 
  \vspace{0.15cm}
  \begin{itemize}
    \item Divide data into two regions:
      $R_1=\{i:X^j_i<s\}$,\quad $R_2=\{i:X^j_i\geq s\}$
    \vspace{0.1cm}
    ...
    \end{align*}
    where $\bar{Y}_{R_k}$ is the mean $Y$ in region $k$
  \end{itemize}
  \vspace{0.15cm}
  \item Choose $(j^*, s^*)$ that gives the \textbf{smallest} $\text{RSS}(j,s)$
  \vspace{0.15cm}
...
\node[font=\scriptsize\bfseries] at (0,2.05) {Full data};
\foreach \i in {1,...,6} {
  \node[obs] at ({(\i-3.5)*0.55},1.5) {\i};
}

\draw[arrow] (-0.9,1.1) -- (-1.7,0.35);
\draw[arrow] (0,1.1) -- (0,0.35);
\draw[arrow] (0.9,1.1) -- (1.7,0.35);

\node[font=\scriptsize] at (-1.7,0.05) {Tree 1};
\foreach \i/\v in {1/1,2/2,3/2,4/4,5/6} {
  \node[samp] at ({-1.7+(\i-3)*0.45},-0.42) {\v};
}

\node[font=\scriptsize] at (0,0.05) {Tree 2};
\foreach \i/\v in {1/1,2/3,3/4,4/4,5/5} {
  \node[samp, fill=green!20] at ({0+(\i-3)*0.45},-0.42) {\v};
}

\node[font=\scriptsize] at (1.7,0.05) {Tree $B$};
\foreach \i/\v in {1/2,2/3,3/5,4/6,5/6} {
  \node[samp, fill=purple!15] at ({1.7+(\i-3)*0.45},-0.42) {\v};
}
\end{tikzpicture}
\end{center}
...
\node[font=\scriptsize\bfseries] at (0,2.05) {All $p$ covariates};
\foreach \x/\name in {-2/age,-1/educ,0/re74,1/re75,2/race} {
  \node[var] at (\x,1.5) {\name};
}
\node at (2.85,1.5) {$\cdots$};

\draw[arrow] (-1.1,1.05) -- (-1.8,0.35);
\draw[arrow] (1.1,1.05) -- (1.8,0.35);

\node[font=\scriptsize] at (-1.8,0.05) {Split in Tree 1};
\node[pick] at (-2.6,-0.42) {age};
\node[pick] at (-1.8,-0.42) {re74};
\node[pick] at (-1.0,-0.42) {educ};

\node[font=\scriptsize] at (1.8,0.05) {Split in Tree 2};
\node[pick, fill=orange!20] at (1.0,-0.42) {race};
\node[pick, fill=orange!20] at (1.8,-0.42) {re75};
\node[pick, fill=orange!20] at (2.6,-0.42) {age};
\end{tikzpicture}
\end{center}
...
\textbf{In each leaf, compare treated vs.\ control:}\quad
$\hat{\tau}(\ell)=\bar{Y}^{1}_{\ell}-\bar{Y}^{0}_{\ell}$
...
\begin{tikzpicture}[scale=0.85, transform shape,
  level 1/.style={sibling distance=3.2cm, level distance=2.0cm},
  internal/.style={draw, rounded corners, fill=orange!25,
...
\node[internal] {age $<30$?}
  child {node[leaf] {$\bar{Y}^{1}=\$11{,}000$\\
                     $\bar{Y}^{0}=\$8{,}000$\\[2pt]
                     $\hat{\tau}=\$3{,}000$}
    edge from parent node[left, draw=none, fill=none,
      font=\scriptsize] {Yes}
  }
  child {node[leaf] {$\bar{Y}^{1}=\$18{,}500$\\
                     $\bar{Y}^{0}=\$18{,}000$\\[2pt]
                     $\hat{\tau}=\$500$}
    edge from parent node[right, draw=none, fill=none,
      font=\scriptsize] {No}
...
\vspace{0.3cm}
\begin{itemize}
  \item Each \textbf{leaf} = a subgroup of workers with similar $x$
  \vspace{0.15cm}
  \item Within each leaf, estimate CATE: treated mean $-$ control mean
  \vspace{0.15cm}
  \item A new worker falls into leaf $\ell(x)$:
  \[
    \hat{\tau}(x)=\hat{\tau}(\ell(x))
  \]
  \vspace{0.05cm}
  \item Workers with similar $x$ share the same estimated effect
\end{itemize}
\end{column}
...
\begin{tikzpicture}[scale=0.68, transform shape,
  box/.style={rectangle, draw, rounded corners, align=center,
              minimum width=2.8cm, minimum height=0.65cm, font=\small},
  arrow/.style={->, >=stealth, thick}
]
\node[box, fill=gray!15, minimum width=3.8cm] (sample) at (0,2.55)
  {Random subsample};

\node[box, fill=blue!12] (split) at (-3.0,1.15)
  {Splitting sample\\learn where to split};
\node[box, fill=green!15] (est) at (3.0,1.15)
  {Estimation sample\\estimate leaf effects};

\node[box, fill=orange!20, minimum width=4.0cm] (tree) at (0,-0.35)
  {Honest causal tree};
...
\node[font=\scriptsize, align=center] at (-3.0,0.25)
...
\begin{enumerate}
  \item For each tree $b = 1, \ldots, B$, independently:
  \vspace{0.05cm}
  \begin{itemize}
    \item Draw a \textbf{random subsample} of observations
    \item At each split node, consider only a \textbf{random subset of covariates}\\
          {\scriptsize (these two sources of randomness make each tree different)}
  \end{itemize}
  \vspace{0.08cm}
  \item Split the subsample into two parts:
  \vspace{0.05cm}
  \begin{itemize}
    \item \textbf{Splitting sample}: find \emph{where} to split ---
          choose splits that maximise treatment-effect heterogeneity
    \item \textbf{Estimation sample}: estimate \emph{how large} the effect is
          in each leaf: $\hat{\tau}(\ell)=\bar{Y}^{1}_{\ell}-\bar{Y}^{0}_{\ell}$
  \end{itemize}
  \vspace{0.08cm}
  \item Grow one honest causal tree $\hat{\tau}_b(x)$
  \vspace{0.08cm}
  \item For a worker with characteristics $x$, average across all $B$ trees:
  \[
    \hat{\tau}(x)
    =
