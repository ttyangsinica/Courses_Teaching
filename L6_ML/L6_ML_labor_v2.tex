...
\begin{frame}{From Regression to High-Dimensional Settings}
		\begin{itemize}
			\item \textbf{Recall from Part I:} Under the CIA, regression controls for observed confounders $X$ to identify the causal effect of treatment $D$
			\begin{eqnarray*}
				Y_{i} = \alpha D_{i} + X_{i}\beta + \epsilon_{i}
			\end{eqnarray*}
			\vspace{0.2cm}
			\item \textbf{Challenge:} To satisfy CIA, we may need to include \textbf{many} covariates $X$
			\vspace{0.2cm}
			\begin{itemize}
				\item Interaction terms, polynomials, and many control variables
				\vspace{0.2cm}
				\item The number of covariates $p$ may be large relative to sample size $n$
			\end{itemize}
			\vspace{0.3cm}
			\item \textbf{Part II:} Post-Double Selection (PDS) uses LASSO to select the most relevant controls from high-dimensional data while preserving valid causal inference
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Problem of High-Dimensional Data}{Example: Effect of Abortion on Crime}
    John J. Donohue and Steven D. Levitt (2001), ``\textbf{The Impact of 
    Legalized Abortion on Crime,}'' \textit{Quarterly Journal of Economics}

    \begin{itemize}
        \item Examine the causal effect of legalized abortion on crime rate
        \vspace{0.2cm}
        \begin{itemize}
            \item \textbf{Mechanism:} unwanted children are at elevated 
            risk of criminal behavior; abortion legalization reduced 
            the number of such births
        \end{itemize}
        \vspace{0.2cm}
        \item Crime began to fall roughly 18 years after abortion 
        legalization (Roe v.\ Wade, 1973)
        \vspace{0.2cm}
        \item States with high abortion rates in the 1970s--80s 
        experienced greater crime reductions in the 1990s
        \vspace{0.2cm}
        \item \textbf{Highly controversial} --- many debates on 
        identification and confounders
    \end{itemize}
\end{frame}


\begin{frame}{Example: Effect of Abortion on Crime}{Setup}
    \begin{itemize}
        \item \textbf{Goal:} estimate causal effect of $d_{it}$ 
        (abortion rate) on $y_{it}$ (crime rate)
        \vspace{0.2cm}
        \item \textbf{Problem:} abortion rates are not randomly assigned
        \vspace{0.2cm}
        \item \textbf{Key concern:} many confounders correlated with 
        both abortion and crime rates
        \begin{itemize}
            \vspace{0.2cm}
            \item States differ in demographics, income, policing, 
            social programs, etc.
            \vspace{0.2cm}
            \item Crime rates evolve differently across states 
            for many reasons
            \vspace{0.2cm}
            \item Example confounder: high school dropout rate 
            --- correlated with both abortion access and crime
        \end{itemize}
        \vspace{0.2cm}
        \item \textbf{Solution:} control for $p = 284$ potential 
        covariates --- but $n = 600$ (50 states $\times$ 12 years)
        \begin{itemize}
            \vspace{0.1cm}
            \item $p$ is large relative to $n$ $\Rightarrow$ 
...
\begin{frame}{Example: Effect of Abortion on Crime}{Baseline Model}
    \begin{itemize}
        \item Donohue and Levitt (2001) baseline model:
        \begin{eqnarray*}
            y_{it} = \alpha_{0} d_{it} + \sum^{p}_{j=1} \beta_{j} 
            x^{j}_{it} + \gamma_{t} + \delta_{i} + \epsilon_{it}
        \end{eqnarray*}
        \begin{itemize}
            \item $n = 600$: 50 states $\times$ 12 years
            \vspace{0.1cm}
            \item $y_{it}$: crime rate (violent, property, or murder 
            per 1,000 people)
            \vspace{0.1cm}
            \item $d_{it}$: ``effective'' abortion rate
            \vspace{0.1cm}
            \item \textbf{$p = 284$ potential covariates} $x^{j}_{it}$: 
            lagged prisoners, lagged police $\times t$, income, 
            beer consumption, initial abortion rate, etc.
            \vspace{0.1cm}
            \item $\gamma_{t}$: time fixed effects; \ 
            $\delta_{i}$: state fixed effects
        \end{itemize}
        % \vspace{0.2cm}
        % \item \textbf{Replication data publicly available:}
        % Belloni, Chernozhukov, and Hansen (2014, \textit{JEP}) 
        % provide Stata code and data at 
        % \href{https://www.openicpsr.org/openicpsr/project/113924/version/V1/view}{openICPSR} 
        % and \href{https://github.com/mattmikem/flp}{GitHub}
    \end{itemize}
\end{frame}
...
\begin{frame}{Using Machine Learning to Improve Causal Inference}{Post-Double Selection Method}
    \begin{itemize}
        \item Suppose we want to estimate the causal effect of treatment $D_{i}$ on outcome $Y_{i}$
        \vspace{0.2cm}
        ...
        \begin{itemize}
            \vspace{0.2cm}
            \item[1] Drop all $X^{j}_{i}$ that are \textbf{statistically insignificant} (e.g., using t-tests)
            %(e.g., using t-tests or information criteria such as AIC/BIC)
            \vspace{0.2cm}
...
\end{frame}
	...
	\begin{itemize}
		...
		\vspace{0.2cm}
		\item This is equivalent to minimizing SSE subject to 
$\sum^{p}_{j=1} |\beta_{j}| \leq s$ for some $s \geq 0$ 
		\vspace{0.2cm}

\item $s$ and $\lambda$ are in one-to-one correspondence
...
\begin{frame}{LASSO Regression}
    \begin{itemize}
        \item There is no closed-form solution for the LASSO estimator
        % \begin{itemize}
            % \vspace{0.1cm}
            % \item Unlike Ridge regression, the penalty $|\beta_j|$ is
            % non-differentiable at $\beta_j = 0$
        % \end{itemize}
        \vspace{0.2cm}
        \item \textbf{Intuition:}
        \begin{itemize}
            \vspace{0.2cm}
            \item Including more regressors incurs a cost: 
            $\lambda \sum^{p}_{j=1} |\beta_{j}|$
            \vspace{0.2cm}
            \item Variables that contribute little to fit are 
            penalized out: LASSO sets $\hat{\beta}_{j} = 0$ 
            for some $j$, performing \textbf{variable selection}
        \end{itemize}
        \vspace{0.2cm}
        \item \textbf{Sparsity assumption:} LASSO works best when 
        only a few $\beta_j \neq 0$ and most $\beta_j = 0$
        \vspace{0.2cm}
        \item \textbf{Choosing $\lambda$} $=$ \textbf{choosing which variables to include}
        \begin{itemize}
            \vspace{0.2cm}
            \item Large $\lambda$: heavier penalty $\Rightarrow$ fewer variables selected
            \vspace{0.2cm}
            \item Small $\lambda$: lighter penalty $\Rightarrow$ more variables retained
            \vspace{0.2cm}
            \item Extremes: $\lambda \to \infty$ sets all $\hat{\beta}_j = 0$; 
            $\lambda \to 0$ recovers OLS
           % \vspace{0.1cm}
           % \item $\lambda$ selected by \textbf{cross-validation} or theory-based 
            criteria 
            %(e.g., $\lambda \propto \sqrt{n \log p}$)
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{How to select $\lambda$}{Overview}
    \begin{itemize}
        \item The \textbf{tuning parameter} $\lambda$ controls the 
        \textbf{strength of penalty} and determines a set of covariates
        \vspace{0.2cm}
        \begin{itemize}
            \item Each tuning parameter value $\lambda$ corresponds to 
            a fitted regression model
            \vspace{0.2cm}
        \end{itemize}
        \item \textbf{Example:} given $p=3$ covariates, different $\lambda$ 
        values yield different models:
        \vspace{0.2cm}
        \begin{center}
        \begin{tabular}{cll}
            \hline
            ...
            \hline
            small & $x^1, x^2, x^3$ & $\hat{y}_i = \hat{\beta}_1 x^1_i + \hat{\beta}_2 x^2_i + \hat{\beta}_3 x^3_i$ \\
            medium & $x^1, x^3$ & $\hat{y}_i = \hat{\beta}_1 x^1_i + \hat{\beta}_3 x^3_i$ \\
            large & $x^1$ & $\hat{y}_i = \hat{\beta}_1 x^1_i$ \\
            $\to \infty$ & (none) & $\hat{y}_i = 0$ \\
            \hline
        \end{tabular}
        \end{center}
        \vspace{0.2cm}
        \item The shrinkage methods allow us to \textbf{simplify the model 
        selection problem to a one-dimensional problem}
    \end{itemize}
\end{frame}
...
\begin{frame}{Three Ways to Choose $\lambda$}{Overview}
    \begin{itemize}
        ...
        \vspace{0.2cm}
        \begin{itemize}
            \item Data-driven: choose $\lambda$ that minimizes 
            \textbf{out-of-sample prediction error} (typically via $k$-fold CV)
            \vspace{0.2cm}
            \item Simple and widely used, but computationally costly 
            and does not guarantee inferential properties for causal estimation
        \end{itemize}
        \vspace{0.2cm}
        \item[2] \textbf{Rigorous (theory-based) approach:}
        \vspace{0.2cm}
        \begin{itemize}
            \item Theory-driven: Belloni, Chernozhukov, and Hansen 
            (2014, \textit{ReStud}) derive the optimal 
            $\lambda \propto \sqrt{n \log p}$
            \vspace{0.2cm}
            \item More data ($\uparrow n$) and more variables ($\uparrow p$) 
            $\Rightarrow$ stronger penalty needed to avoid false selection
            \vspace{0.2cm}
            \item Provides formal statistical guarantees in high dimensions; 
            used in the \textbf{Post-Double Selection} method
        \end{itemize}
        % \vspace{0.2cm}
        % \item[3] \textbf{Information criteria approach:} see next slide
    \end{itemize}
\end{frame}

\begin{frame}{Three Ways to Choose $\lambda$}{Information Criteria Approach}
    \begin{itemize}
        \item[3] \textbf{Information criteria approach:}
        \vspace{0.2cm}
        \begin{itemize}
            \item Choose $\lambda$ that minimizes an information criterion, 
            e.g., BIC:
            \vspace{0.2cm}
            \begin{eqnarray*}
                \text{BIC}(\lambda) = \log\left(\frac{\text{SSE}(\lambda)}{n}\right) 
                + \frac{\log n}{n} \cdot \hat{s}(\lambda)
            \end{eqnarray*}
            \vspace{0.1cm}
            \item $\hat{s}(\lambda)$: number of selected variables 
            (non-zero $\hat{\beta}_j$) under penalty $\lambda$
            \vspace{0.2cm}
            \item \textbf{Intuition:} trade-off between fit and sparsity
            \begin{itemize}
                \vspace{0.1cm}
                \item Large $\lambda$ $\Rightarrow$ small $\hat{s}(\lambda)$ 
                but large SSE
                \vspace{0.1cm}
                \item Small $\lambda$ $\Rightarrow$ small SSE 
                but large $\hat{s}(\lambda)$
                \vspace{0.1cm}
                \item BIC selects $\lambda$ that balances the two
            \end{itemize}
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Cross-validation approach}{Training Data}
    \begin{itemize}
        \item Use \textbf{training data} to estimate the following 
        regression using LASSO:
        \begin{eqnarray*}
            y_{i} = \sum^{p}_{j=1} \beta_{j} x^{j}_{i} + \epsilon_{i}, 
            \ \ i=1, \ldots, n
        \end{eqnarray*}
        \vspace{0.1cm}
        \item Choose $(\hat{\beta}_{1}, \hat{\beta}_{2}, \ldots, \hat{\beta}_{p})$ 
        that minimize \textbf{SSE with a penalty for model size}:
        \begin{eqnarray*}
            \sum^{n}_{i=1} \left(y_{i} - \sum^{p}_{j=1} \beta_{j} 
            x^{j}_{i}\right)^{2} + \lambda \sum^{p}_{j=1} |\beta_{j}|
        \end{eqnarray*}
        \begin{itemize}
            \item where $\lambda \geq 0$ is a \textbf{tuning parameter}
        \end{itemize}
        \vspace{0.2cm}
        \item Predict $y_{i}$ using the estimated model 
        $\hat{f}_{\lambda}(\cdot)$:
        \begin{eqnarray*}
            \hat{y}_{i} = \hat{f}_{\lambda}(x^{j}_{i}) 
            = \sum^{p}_{j=1} \hat{\beta}_{j} x^{j}_{i}
        \end{eqnarray*}
    \end{itemize}
\end{frame}
...
\begin{frame}{Cross-validation approach}{Validation Data}
    \begin{itemize}
        \item Use the estimated model $\hat{f}_{\lambda}(\cdot)$ to predict 
        unseen observations in the \textbf{validation data}
        \vspace{0.2cm}
        \item Evaluate prediction accuracy via \textbf{SSE} or \textbf{MSE}:
        \begin{eqnarray*}
            \text{SSE} = \sum^{n}_{i=1} 
            \left(y^{+}_{i} - \hat{f}_{\lambda}(x^{j+}_{i})\right)^2
        \end{eqnarray*}
        \begin{eqnarray*}
            \text{MSE} = \frac{1}{n} \sum^{n}_{i=1} 
            \left(y^{+}_{i} - \hat{f}_{\lambda}(x^{j+}_{i})\right)^2
        \end{eqnarray*}
        \begin{itemize}
            \item $(x^{j+}_{i}, y^{+}_{i})$: unseen observations 
            in \textbf{validation data}
            \vspace{0.1cm}
            \item $\hat{f}_{\lambda}(\cdot)$: estimated model from 
            \textbf{training data}
        \end{itemize}
        \vspace{0.2cm}
        \item \textbf{Key idea:} a model that fits training data well may 
        not predict validation data well $\Rightarrow$ \textbf{overfitting}
    \end{itemize}
\end{frame}



% \begin{frame}{Cross-validation approach}{Training vs.\ Validation MSE}
% \begin{center}
% \begin{tikzpicture}[scale=1.0]

% \draw[->, thick] (0,0) -- (7.5,0) node[right] {$\lambda$};
% \draw[->, thick] (0,0) -- (0,5) node[above] {MSE};

% \draw[blue, thick, domain=0.3:7, smooth, samples=100]
%   plot (\x, {0.3 + 0.08*\x*\x});

% \draw[red, dashed, thick, domain=0.3:7, smooth, samples=100]
%   plot (\x, {3.5 + 2.5*exp(-1.2*\x) + 0.08*(\x-2.5)*(\x-2.5) - 0.3});

% \draw[dashed, gray] (2.5, 0) -- (2.5, 3.2);
% \node[below] at (2.5, 0) {$\lambda^{*}$};
% \filldraw[green!60!black] (2.5, 3.2) circle (2.5pt);

% \node[blue, right] at (7, 4.3) {Training MSE};
% \node[red, right] at (5.5, 4.0) {Validation MSE};

% \node[gray, font=\small] at (1.0, -0.5) {overfitting};
% \node[gray, font=\small] at (5.5, -0.5) {underfitting};

% \draw[<->, gray] (0.1, -0.35) -- (2.4, -0.35);
% \draw[<->, gray] (2.6, -0.35) -- (6.9, -0.35);

% \end{tikzpicture}
% \end{center}
% \begin{itemize}
%     \item \textbf{Choose $\lambda^{*}$} that minimizes MSE on \textbf{validation data}
%     \item Small $\lambda$: low Training MSE but high Validation MSE 
%     $\Rightarrow$ \textbf{overfitting}
%     \item Large $\lambda$: both MSEs rise $\Rightarrow$ \textbf{underfitting}
% \end{itemize}
% \end{frame}
...
\begin{frame}{K-fold Cross Validation}{Step 2}
	\begin{itemize}
	\item[2] For each value of the tuning parameter $\lambda \in \{\lambda_{1},\ldots,\lambda_{m} \}$, we do the following steps: 
				\vspace{0.2cm}
		\begin{itemize}
...
\begin{frame}{K-fold Cross Validation}{Step 3: Choose $\hat{\lambda}$}
    \begin{itemize}
        \item[3] Choose $\lambda$ that minimizes the \textbf{cross-validation 
        error curve} $CV(\lambda)$:
        \begin{eqnarray*}
            \hat{\lambda} = \arg \min_{\lambda \in \{\lambda_{1},\ldots,\lambda_{m}\}} 
            CV(\lambda)
        \end{eqnarray*}
        \vspace{0.1cm}
        \item $CV(\lambda)$ is a U-shaped curve 
        \vspace{0.2cm}
        \item When $K = n$: \textbf{leave-one-out cross-validation}
       
    \end{itemize}
\end{frame}

\begin{frame}{K-fold Cross Validation}{CV Error Curve}
    \begin{center}
    \begin{tikzpicture}[scale=1.05]

    \draw[->, thick] (0,0) -- (7.5,0) node[right] {$\lambda$};
    \draw[->, thick] (0,0) -- (0,5) node[above] {$CV(\lambda)$};

    \draw[red, dashed, thick, domain=0.3:7, smooth, samples=100]
        plot (\x, {3.5 + 2.5*exp(-1.2*\x) + 0.08*(\x-2.5)*(\x-2.5) - 0.3});

    \draw[dashed, gray] (2.5, 0) -- (2.5, 3.2);
    \node[below] at (2.5, 0) {$\hat{\lambda}$};
    \filldraw[green!60!black] (2.5, 3.2) circle (2.5pt);
    \node[green!60!black, above right, font=\small] at (2.6, 3.3) 
        {$\min CV(\lambda)$};

    \node[red] at (6.2, 4.5) {};

    \node[gray, font=\small] at (1.0, -0.6) {overfitting};
    \node[gray, font=\small] at (5.5, -0.6) {underfitting};

    \draw[<->, gray] (0.1, -0.45) -- (2.4, -0.45);
    \draw[<->, gray] (2.6, -0.45) -- (6.9, -0.45);
...
\begin{frame}{STATA Example: Test the Convergence Hypothesis}{Overview}
	Sala-i-Martin, Xavier (1997), ``\textbf{I Just Ran Two Million Regressions}", \textit{American Economic Review} (cited 2,931 times)
	\vspace{0.3cm}
	\begin{itemize}
		\item Same setup as Example 1: test $\alpha < 0$ (convergence hypothesis) in
		\begin{eqnarray*}
			\underbrace{\mbox{GrowthRate}}_{Y_{i}} = \beta_{0} + \underbrace{\alpha}_{\mbox{ATE}} \underbrace{\log(\mbox{GDP})}_{D_{i}} + \sum^{p}_{j=1} \beta_{j} X^{j}_{i} + \epsilon_{i}
		\end{eqnarray*}
		\item Data: $p = 60$ covariates, $n = 90$ observations $\Rightarrow$ need \textbf{variable selection}
		\vspace{0.2cm}
		\item Key finding: a large number of variables are robustly related to growth
	\end{itemize}
\end{frame}
...
% ---- NEW STATA EXAMPLE: Abortion & Crime (Donohue & Levitt 2001) ------------

\begin{frame}{STATA Example: Abortion and Crime (Donohue \& Levitt 2001)}{Overview}
	Donohue, J.J. and Levitt, S.D. (2001), ``\textbf{The Impact of Legalized Abortion on Crime}", \textit{Quarterly Journal of Economics}
	\vspace{0.3cm}
	\begin{itemize}
		\item Does legalizing abortion reduce crime 15--20 years later?
		\vspace{0.2cm}
		\item First-difference model (removes state fixed effects):
		\begin{eqnarray*}
			\Delta \underbrace{\log(\text{ViolentCrime})}_{Y_{it}} = \underbrace{\alpha}_{\text{ATE}} \cdot \Delta \underbrace{\text{AbortionRate}}_{D_{it}} + \sum^{p}_{j=1} \beta_{j} \Delta X^{j}_{it} + \eta_{t} + \epsilon_{it}
		\end{eqnarray*}
		\item Data: 50 states $\times$ 12 years (1986--1997); 8 base controls expanded to $p = 44$ via squares \& interactions + year FE
		\vspace{0.2cm}
		\item Key question: does $\hat{\alpha} < 0$? (higher abortion $\Rightarrow$ lower crime)
	\end{itemize}
\end{frame}



\begin{frame}[fragile]{STATA Example: Abortion and Crime}{Data and Code}
	\begin{itemize}
	   \item See \textbf{ML\_v2.do}
		\vspace{0.2cm}
		\item Use \textbf{levitt\_ex.dat}
		\vspace{0.2cm}
		\item 8 base controls: prison, police, unemployment, income, poverty, AFDC, gun laws, beer tax
		\vspace{0.2cm}
		\item Expand to \textbf{44 controls}: 8 first differences + 8 squared terms + 28 pairwise interactions
	\end{itemize}
\end{frame}



\begin{frame}[fragile]{STATA Example: Abortion and Crime}{Feature Engineering}
\begin{lstlisting}
local Dlist "Dxxprison Dxxpolice Dxxunemp Dxxincome ///
    Dxxpover Dxxafdc15 Dxxgunlaw Dxxbeer"

* Squared differences (8 vars)
foreach x of local Dlist { gen `x'2 = `x'^2 }

* Pairwise interactions (28 vars)
local n : word count `Dlist'
forvalues i = 1/`n' {
    local start = `i' + 1
    forvalues j = `start'/`n' {
        local vi : word `i' of `Dlist'
        local vj : word `j' of `Dlist'
        gen `vi'X`vj' = `vi' * `vj'
    }
}

local AllCtrl `Dlist' `Dsq' `Dint'   * 44 controls total
\end{lstlisting}
\end{frame}



\begin{frame}[fragile]{STATA Example: Abortion and Crime}{Control All Covariates}
\begin{lstlisting}
reg Dyviol Dviol `AllCtrl' _Iyear*, cluster(statenum)
\end{lstlisting}
\begin{itemize}
	\item OLS with all 44 controls + year dummies (benchmark)
	\vspace{0.2cm}
	\item With $n \approx 600$ obs.\ and $p = 44$ controls, overfitting is a real concern
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{STATA Example: Abortion and Crime}{Control All Covariates}
		\begin{figure}[p]
			\centering
			\includegraphics[width=.9\textwidth]{figure/abort_scatter.png}
		\end{figure}
\end{frame}
...
\begin{itemize}
\normalsize
\item Change in abortion rate is negatively associated with change in violent crime
\vspace{0.2cm}
\item But OLS with all 44 controls is unreliable: imprecise estimates, wide confidence intervals
\vspace{0.2cm}
\item With $p = 44$ controls and $n \approx 600$, we need \textbf{variable selection} to identify the most relevant controls
\end{itemize}
\end{frame}
...
% ---- NEW STATA PDS SECTION --------------------------------------------------

\begin{frame}[fragile]{STATA Example: Abortion and Crime}{Post-Double Selection LASSO}
\begin{lstlisting}
pdslasso Dyviol Dviol ///
    (`AllCtrl' _Iyear*), ///
    pnotpen(_Iyear*) cluster(statenum)
\end{lstlisting}
\begin{itemize}
	\item \textbf{pdslasso}: double selection LASSO for causal inference
	\vspace{0.2cm}
	\begin{itemize}
		\item \textbf{AllCtrl}: all 44 penalized controls (LASSO selects the relevant ones)
		\vspace{0.2cm}
		\item \textbf{pnotpen(\_Iyear*)}: year dummies always included (not penalized)
		\vspace{0.2cm}
		\item \textbf{cluster(statenum)}: cluster-robust standard errors
	\end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
\normalsize
\item PDS-LASSO selects controls correlated with the outcome or the treatment
\vspace{0.2cm}
\item Estimated $\hat{\alpha} < 0$: higher abortion rate $\Rightarrow$ lower violent crime 15--20 years later
\vspace{0.2cm}
\item Coefficient is \textbf{statistically significant}: supports Donohue \& Levitt's original finding
\end{itemize}
\end{frame}



\begin{frame}[fragile]{STATA Example: Abortion and Crime}{Manual PDS Steps}
\begin{lstlisting}
* Step 1: LASSO - select controls predicting outcome
rlasso Dyviol Dxxprison Dxxpolice Dxxunemp Dxxincome Dxxpover ///
    Dxxafdc15 Dxxgunlaw Dxxbeer _Iyear*, pnotpen(_Iyear*) rob

* Step 2: LASSO - select controls predicting treatment
rlasso Dviol Dxxprison Dxxpolice Dxxunemp Dxxincome Dxxpover ///
    Dxxafdc15 Dxxgunlaw Dxxbeer _Iyear*, pnotpen(_Iyear*) rob

* Step 3: OLS with union of selected controls
* reg Dyviol Dviol [...] _Iyear*, cluster(statenum)
\end{lstlisting}
\begin{itemize}
	\item Manual implementation shows which variables each LASSO step selects
\end{itemize}
\end{frame}



\begin{frame}[fragile]{STATA Example: Abortion and Crime}{Visualization}
\begin{lstlisting}
graph twoway ///
    (scatter Dyviol Dviol) ///
    (lfit Dyviol Dviol), ///
    title("Change in Violent Crime vs. Abortion Rate") ///
    xtitle("Change in Abortion Rate (efaviol)") ///
    ytitle("Change in Log Violent Crime") ///
    graphregion(color(white))

graph export "$figure/abort_scatter.png", replace width(3000)
\end{lstlisting}
\end{frame}
...
\begin{frame}[fragile]{STATA Example: Abortion and Crime}{Partialed-Out Scatter}
\begin{lstlisting}
regress Dyviol Dxxprison Dxxpolice Dxxunemp Dxxincome Dxxpover ///
    Dxxafdc15 Dxxgunlaw Dxxbeer _Iyear*
predict res_Dyviol, residuals

regress Dviol Dxxprison Dxxpolice Dxxunemp Dxxincome Dxxpover ///
    Dxxafdc15 Dxxgunlaw Dxxbeer _Iyear*
predict res_Dviol, residuals
\end{lstlisting}
\begin{itemize}
	\item Partial out controls to isolate the \textbf{causal channel} (FWL theorem)
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{STATA Example: Abortion and Crime}{Partialed-Out Scatter}
		\begin{figure}[p]
			\centering
			\includegraphics[width=.9\textwidth]{figure/abort_scatter_r.png}
		\end{figure}
\end{frame}
...
\begin{frame}[fragile]{R Example: Abortion and Crime}{Data and Code}
	\begin{itemize}
		\item See \textbf{ML\_v2.R}
		\vspace{0.2cm}
		\item Use \textbf{levitt\_ex.dat}
		\vspace{0.2cm}
		\item 8 base controls expanded to \textbf{44 controls}: 8 diffs + 8 squares + 28 interactions
		\vspace{0.2cm}
		\item Install: haven, hdm, ggplot2, dplyr
	\end{itemize}
\end{frame}



\begin{frame}[fragile]{R Example: Abortion and Crime}{Read and Prepare Data}
\begin{lstlisting}
library(hdm); library(haven); library(ggplot2); library(dplyr)

AbortData <- read.table(paste0(workdata, "/levitt_ex.dat"),
                        header = TRUE, sep = "\t")

AbortData <- AbortData %>%
  filter(statenum != 9, year >= 85, year <= 97) %>%
  mutate(xxincome = xxincome / 100,
         xxpover  = xxpover  / 100,
...
\end{lstlisting}
\begin{itemize}
	\item Drop DC, keep 1985--1997, rescale controls
\end{itemize}
\end{frame}



\begin{frame}[fragile]{R Example: Abortion and Crime}{Generate First Differences}
\begin{lstlisting}
ctrl_vars <- c("xxprison","xxpolice","xxunemp","xxincome",
               "xxpover","xxafdc15","xxgunlaw","xxbeer")

AbortData <- AbortData %>%
  group_by(statenum) %>%
  mutate(
    Dyviol = lpc_viol - lag(lpc_viol),
    Dviol  = efaviol  - lag(efaviol),
    across(all_of(ctrl_vars), ~ . - lag(.), .names = "D{.col}")
  ) %>%
  ungroup() %>%
  filter(!is.na(Dyviol))
\end{lstlisting}
\begin{itemize}
	\item \textbf{group\_by + lag()}: panel first differences within each state
\end{itemize}
\end{frame}



\begin{frame}[fragile]{R Example: Abortion and Crime}{Feature Engineering}
\begin{lstlisting}
ctrl_D <- paste0("D", ctrl_vars)

# Squared differences (8 vars)
for (v in ctrl_D) AbortData[[paste0(v,"2")]] <- AbortData[[v]]^2
ctrl_D_sq <- paste0(ctrl_D, "2")

# Pairwise interactions (28 vars)
ctrl_D_int <- c()
for (i in 1:(length(ctrl_D)-1)) {
  for (j in (i+1):length(ctrl_D)) {
    nm <- paste0(ctrl_D[i],"X",ctrl_D[j])
    AbortData[[nm]] <- AbortData[[ctrl_D[i]]] * AbortData[[ctrl_D[j]]]
    ctrl_D_int <- c(ctrl_D_int, nm)
  }
}
all_ctrl <- c(ctrl_D, ctrl_D_sq, ctrl_D_int)  # 44 controls
\end{lstlisting}
\end{frame}



\begin{frame}[fragile]{R Example: Abortion and Crime}{Post-Double Selection Estimation}
\begin{lstlisting}
year_mat <- model.matrix(~ as.factor(year) - 1, data = AbortData)[, -1]

X <- cbind(as.matrix(AbortData[, all_ctrl, drop = FALSE]), year_mat)
y <- as.matrix(AbortData$Dyviol)
d <- as.matrix(AbortData$Dviol)

doublesel.effect <- rlassoEffect(x = X, y = y, d = d,
                                  method = "double selection")
summary(doublesel.effect)
confint(doublesel.effect)
\end{lstlisting}
\begin{itemize}
	\item \textbf{rlassoEffect()}: selects from 44 controls via PDS
\end{itemize}
\end{frame}



\begin{frame}[fragile]{R Example: Abortion and Crime}{Scatter Plot}
\begin{lstlisting}
ggplot(AbortData, aes(x = Dviol, y = Dyviol)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(title = "Change in Violent Crime vs. Abortion Rate",
       x = "Change in Abortion Rate (efaviol)",
       y = "Change in Log Violent Crime") +
  theme(plot.background = element_rect(fill = "white"))

ggsave(filename = file.path(figure, "abort_scatter.png"),
       width = 10, dpi = 300)
\end{lstlisting}
\end{frame}
...
\begin{frame}[fragile]{R Example: Abortion and Crime}{Partialed-Out Scatter}
\begin{lstlisting}
ctrl_formula <- paste(c(all_ctrl, "as.factor(year)"), collapse = " + ")
model_y <- lm(as.formula(paste("Dyviol ~", ctrl_formula)), data = AbortData)
AbortData$res_Dyviol <- resid(model_y)

model_d <- lm(as.formula(paste("Dviol ~", ctrl_formula)), data = AbortData)
AbortData$res_Dviol <- resid(model_d)

ggplot(AbortData, aes(x = res_Dviol, y = res_Dyviol)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(title = "Partialed Scatter: Violent Crime vs. Abortion Rate",
       x = "Residual Abortion Rate",
       y = "Residual Log Violent Crime") +
  theme(plot.background = element_rect(fill = "white"))

ggsave(filename = file.path(figure, "abort_scatter_r.png"), width = 10, dpi = 300)
\end{lstlisting}
\end{frame}
	...
	\vspace{0.2cm}
		...
		\begin{itemize}
			...
			\item Video lectures:  
							\vspace{0.2cm} 
								\begin{itemize}
			\item \url{https://www.youtube.com/playlist?list=PLOg0ngHtcqbPTlZzRHA2ocQZqB1D_qZ5V}
									\end{itemize}
		\end{itemize}
