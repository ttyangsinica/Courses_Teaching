...
\begin{frame}{Identification Results for IV}
	\begin{itemize}
		...
		\begin{equation*}
			...
			\end{aligned}
		\end{equation*}
		\item Formal representation (the \textbf{Wald estimator}):
		\begin{equation*}
			\alpha_{IV}=\dfrac{\mathrm{E}[Y_{i}|Z_{i}=1]-\mathrm{E}[Y_{i}|Z_{i}=0]}{\mathrm{E}[D_{i}|Z_{i}=1]-\mathrm{E}[D_{i}|Z_{i}=0]}
		\end{equation*}

		\item This ratio has a natural two-part interpretation:
				\vspace{0.2cm}
		\begin{itemize}
			\item \textbf{Reduced-form} (numerator): total effect of the instrument on the outcome
				\vspace{0.2cm}
			\item \textbf{First-stage} (denominator): how strongly the instrument shifts treatment take-up
				\vspace{0.2cm}
			\item Dividing ``undilutes'' the effect --- because the lottery moves only \emph{some} people into treatment, we scale up to recover the per-person causal effect for those actually affected
		\end{itemize}
...
\begin{frame}{Identification Assumptions for IV}
	\begin{itemize}
		\item The Wald estimator identifies a causal effect only when certain conditions hold
		\vspace{0.2cm}
		\item We need \textbf{four identification assumptions}:
		\vspace{0.2cm}
		\begin{enumerate}
			\item \textbf{First-stage}: The instrument $Z_i$ must actually affect treatment $D_i$
			\vspace{0.2cm}
			\item \textbf{Independence}: The instrument is as good as randomly assigned
			\vspace{0.2cm}
			\item \textbf{Exclusion Restriction}: The instrument affects outcomes \emph{only} through the treatment
			\vspace{0.2cm}
			\item \textbf{Monotonicity}: The instrument moves everyone in the same direction (no defiers)
		\end{enumerate}
	\end{itemize}
\end{frame}
...
\begin{frame}{Identification Results for IV}
	\begin{itemize}
		...
		\item Under the four identification assumptions, we can prove that the Wald estimator identifies the \textbf{Local Average Treatment Effect (LATE)}

\begin{block}{IV Identifies LATE}
	\begin{equation*}
		\alpha_{IV}=\dfrac{\mathrm{E}[Y_{i}|Z_{i}=1]-\mathrm{E}[Y_{i}|Z_{i}=0]}{\mathrm{E}[D_{i}|Z_{i}=1]-\mathrm{E}[D_{i}|Z_{i}=0]}
...
\end{block}
		...
		\vspace{0.2cm}
		\item The IV estimator identifies the causal effect \emph{only} for compliers
		\vspace{0.2cm}
		\item Lottery IV identifies the causal effect of military service on lifetime earnings for those who follow the lottery result (e.g.\ David and Tim)
	\end{itemize}
\end{frame}
