...
\vspace{0.12cm}
\begin{enumerate}
  \item For each variable $X^j$ and split point $s$:
  \vspace{0.08cm}
  \begin{itemize}
    \item Divide data into two regions:\\
      $R_1=\{i:X^j_i<s\}$,\quad $R_2=\{i:X^j_i\geq s\}$
    \vspace{0.06cm}
    \item Compute \textbf{RSS} after the split:
    \[
      \text{RSS}(j,s)=
    ...
    \]
    \vspace{-0.1cm}
    {\scriptsize where $\bar{Y}_{R_k}$ = mean $Y$ in region $k$}
  \end{itemize}
  \vspace{0.08cm}
  \item Choose $(j^*, s^*)$ with \textbf{smallest} $\text{RSS}(j,s)$
  \vspace{0.08cm}
...
\vspace{0.15cm}
\begin{center}
{\small\textit{Example: first split on age $<30$}}
\vspace{0.25cm}

\begin{tikzpicture}[
  level 1/.style={sibling distance=2.5cm, level distance=1.75cm},
  ...
  leaf/.style={draw, rounded corners, fill=green!20,
    align=center, font=\scriptsize,
    minimum width=1.85cm, minimum height=0.9cm},
  edge from parent/.style={draw, -stealth, thick}
]
\node[internal] {age $<30$?}
  child {node[leaf] {$R_1$\\ $\bar{Y}_{R_1}=\$8{,}200$}
    edge from parent node[left, draw=none, fill=none,
      font=\scriptsize] {Yes}
  }
  child {node[leaf] {$R_2$\\ $\bar{Y}_{R_2}=\$11{,}000$}
...
\vspace{0.2cm}
{\scriptsize Minimise total scatter\\[2pt] within $R_1$ and $R_2$}
\end{center}
\end{column}
