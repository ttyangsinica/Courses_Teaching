...
\begin{frame}[fragile]{Reproducible Research: \texttt{.stmd} File Structure (2)}
	...
	\begin{itemize}
		...
		\vspace{0.2cm}
		\item \textbf{Inline value} (\texttt{`s \textit{expr}`}): embed a computed result directly in a sentence
	\end{itemize}
...
\end{lstlisting}
	...
	\begin{itemize}
		\item The inline expression \texttt{`s \%12.0fc \_b[college]`} inserts the estimated coefficient directly into the sentence --- updates automatically when the model changes
		\vspace{0.1cm}
...
\begin{frame}[fragile]{Reproducible Research: Key Syntax Elements}
	...
	\begin{tabular}{lll}
		...
		\texttt{`s \textit{expr}`}           & Embed value \textbf{inline} in text & report coefficients \\
		\hline
	\end{tabular}
...
\begin{frame}[fragile]{Reproducible Research: \texttt{.Rmd} File Structure (2)}
	...
	\begin{itemize}
		...
		\vspace{0.2cm}
		\item \textbf{Inline value} (\texttt{`r \textit{expr}`}): embed a computed result directly in a sentence
	\end{itemize}
...
\end{lstlisting}
	...
	\begin{itemize}
		\item The inline expression \texttt{`r round(coef(model3)["college"], 0)`} inserts the estimated coefficient directly into the sentence --- updates automatically when data or model changes
	\end{itemize}
