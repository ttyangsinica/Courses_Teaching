...
\begin{frame}{Main Idea of Matching}
	\begin{itemize}
		...
		\vspace{0.2cm}
\begin{itemize}
    \item Older workers are \emph{less likely to seek job training}
    \vspace{0.15cm}
    \item Older workers \emph{also earn more} due to their experience
\end{itemize}
					\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Conditional Independence Assumption}{Intuition}
...
\item A naive comparison mixes up the \textbf{effect of job training} with the \textbf{effect of age}.
\end{itemize}

\vspace{0.2cm}
\begin{alertblock}{Solution}
\textbf{Match by age:} Once we compare people of the \textbf{same age}, the confounding effect of age disappears.
\end{alertblock}
...
%% ── 第二頁：正式定義────────────────────────────────────────────────────────
\begin{frame}{Conditional Independence Assumption}{Formal Definition}

\begin{block}{Conditional Independence Assumption (CIA)}
\begin{equation*}
(\mathrm{Y}^{1}_{i},\mathrm{Y}^{0}_{i})\indep D_{i}\mid X_{i}
\end{equation*}
\textit{``Given $X_i$, the potential outcomes are independent of treatment assignment.''}
\end{block}

\vspace{0.3cm}
\begin{itemize}
    \item $(\mathrm{Y}^{1}_{i},\mathrm{Y}^{0}_{i})$: potential outcomes (with/without treatment)
    \vspace{0.2cm}
    \item $D_{i}$: treatment indicator ($1$ = treated, $0$ = control)
    \vspace{0.2cm}
    \item $X_{i}$: observable characteristics (covariates)
    \vspace{0.2cm}
    \item Conditional on $X_i$, treatment assignment becomes \textbf{``as good as random''}
    \vspace{0.2cm}
    \item This assumption is also called \textbf{selection on observables} — all confounding
    comes from observable $X_i$, not hidden factors
\end{itemize}
...
%% ── 第三頁：反事實的邏輯────────────────────────────────────────────────────
% \begin{frame}{Conditional Independence Assumption}{Counterfactual Logic}

% \textbf{Key insight:} Once we control for age, treated and untreated groups are comparable $\Rightarrow$ \textbf{apple-to-apple comparison}

% \vspace{0.4cm}
% This allows us to estimate \textbf{counterfactuals} using the other group (same age):

% \vspace{0.3cm}
% \begin{itemize}
%     \item \textcolor{red}{$\mathrm{E}[Y^{0}_{i}\mid X_i{=}40, D_i{=}1]$} $= \mathrm{E}[Y^{0}_{i}\mid X_i{=}40, D_i{=}0]$
%     \vspace{0.15cm}
%     \begin{itemize}
%         \item What would \textbf{trainees} have earned \emph{without} training?\\
%         $\rightarrow$ Estimated by observed earnings of \textbf{non-trainees} of the same age
%     \end{itemize}
%     \vspace{0.3cm}
%     \item \textcolor{red}{$\mathrm{E}[Y^{1}_{i}\mid X_i{=}40, D_i{=}0]$} $= \mathrm{E}[Y^{1}_{i}\mid X_i{=}40, D_i{=}1]$
%     \vspace{0.15cm}
%     \begin{itemize}
%         \item What would \textbf{non-trainees} have earned \emph{with} training?\\
%         $\rightarrow$ Estimated by observed earnings of \textbf{trainees} of the same age
%     \end{itemize}
% \end{itemize}
...
\begin{tabular}{lcc}
\toprule
 & \textbf{Trained} ($D=1$) & \textbf{Not Trained} ($D=0$) \\
\midrule
\textbf{Observed} &
  $\mathrm{E}[Y^1_i \mid X_i{=}40, D_i{=}1]$ &
  $\mathrm{E}[Y^0_i \mid X_i{=}40, D_i{=}0]$ \\[0.2cm]
\textbf{Counterfactual} &
  \textcolor{red}{$\mathrm{E}[Y^0_i \mid X_i{=}40, D_i{=}1]$} &
  \textcolor{red}{$\mathrm{E}[Y^1_i \mid X_i{=}40, D_i{=}0]$} \\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.2cm}
Under CIA, \textcolor{red}{red cells} (counterfactuals) can be filled in using the \textbf{observed outcomes of the other group}:
...
\end{itemize}
% \begin{itemize}
%     \item \textcolor{red}{$\mathrm{E}[Y^0_i \mid X_i{=}40, D_i{=}1]$} $= \mathrm{E}[Y^0_i \mid X_i{=}40, D_i{=}0]$
%     \vspace{0.1cm}
%     \item \textcolor{red}{$\mathrm{E}[Y^1_i \mid X_i{=}40, D_i{=}0]$} $= \mathrm{E}[Y^1_i \mid X_i{=}40, D_i{=}1]$
% \end{itemize}

% \vspace{0.2cm}
% $\Rightarrow$ CIA turns an \textbf{identification problem} into a \textbf{comparison problem}.
...
\begin{frame}{Conditional Independence Assumption (CIA)}{Violation Example}
	\begin{itemize}
\item CIA requires \textbf{all} confounders to be observable. If a confounder is \textbf{unobserved}, CIA fails.
		\vspace{0.2cm}
	    \item Suppose \textbf{motivation} affects both training enrollment and wage potential, but is \textbf{unobserved}:

		\vspace{0.2cm}
		\begin{itemize}
			\item Highly motivated individuals are more likely to enroll in training 
			\vspace{0.2cm}
			\item These same individuals would earn more regardless of training
			\vspace{0.2cm}
	\item The two groups are \textbf{no longer comparable} even at the same age:
        \[
            \mathrm{E}[Y^{0}_{i}\mid X_i{=}40, D_i{=}1] > \mathrm{E}[Y^{0}_{i}\mid X_i{=}40, D_i{=}0]
        \]
		\end{itemize}
...
CIA fails when there are \textbf{unobservable confounders} — even after controlling for $X_i$, the estimate still suffers from \textbf{selection bias}.
\end{alertblock}
...
\begin{itemize}
    ...
    \begin{equation*}
        \begin{aligned}
            ...
            &= \mathrm{E}[\mathrm{Y}_{i}|X_{i}=28, D_{i}=1] - \mathrm{E}[\mathrm{Y}_{i}|X_{i}=28, D_{i}=0] \\
            &= \dfrac{(17700-8800)+(11500-8800)+(16300-8800)}{3} \\
            &= 15166.67 - 8800 \\
            &= 6{,}366.67 \\[10pt]
            & \mathrm{E}[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}|X_{i}=34, D_{i}=1] \\
            &= \mathrm{E}[\mathrm{Y}_{i}|X_{i}=34, D_{i}=1] - \mathrm{E}[\mathrm{Y}_{i}|X_{i}=34, D_{i}=0] \\
            &= \dfrac{10200-24200}{1} \\
            &= -14{,}000 \quad \cdots
        \end{aligned}
    \end{equation*}
...
\begin{itemize}
    ...
    \item A valid method to calculate standard errors when using estimated
    propensity scores was formally derived by Abadie and Imbens (2016)
    \vspace{0.2cm}
    \item Abadie, Alberto, and Guido W. Imbens. ``\textbf{Matching on the
    Estimated Propensity Score}.'' \textit{Econometrica} 84.2 (2016): 781--807.
    \vspace{0.2cm}
    \begin{itemize}
        \item Need to account for the fact that propensity scores are
        \textbf{estimated}, not fixed and known constants
        \vspace{0.2cm}
        \begin{itemize}
            \item For ATE: The adjustment is always \textbf{negative} —
            standard errors are smaller than if we treated $\hat{p}(X_i)$
            as fixed constants
            \vspace{0.2cm}
            \item For ATT: The adjustment can be \textbf{positive or negative}
            — standard errors can be either smaller or larger
            \vspace{0.2cm}
        \end{itemize}
        \item Ignoring this adjustment can lead to \textbf{incorrect inference}
    \end{itemize}
...
\begin{frame}{Empirical Analysis Workflow}{}
\begin{center}
\renewcommand{\arraystretch}{1.5}
\scalebox{0.88}{
\begin{tabular}{clp{5.5cm}}
\toprule
\textbf{Step} & \textbf{Task} & \textbf{What to Do} \\
\midrule
1 & \textbf{Organize project}
  & Set up folder structure, path setup block \\
2 & \textbf{Read the codebook}
  & Understand variables, units, known data issues \\
3 & \textbf{Clean data}
  & Label variables/values, handle missing values and outliers \\
4 & \textbf{Examine data}
  & Examine distributions, means, and frequencies of key variables \\
5 & \textbf{Pre-analysis balance check}
  & Compare treated vs.\ control group \emph{before} any analysis \\
6 & \textbf{Proceed to analysis}
  & PSM, regression, DiD, \ldots \\
\bottomrule
\end{tabular}}
\end{center}
\vspace{0.15cm}
% \begin{center}
% \small Steps 1--4 apply to \textbf{any} empirical project, not just matching.\\
% AI tools (e.g.\ ChatGPT, Claude) can assist at every step.
% \end{center}
\end{frame}


\begin{frame}{Before You Start: Organize Your Project}{}
\begin{itemize}
    \item Set up a \textbf{clear folder structure} before touching any data:
    \vspace{0.15cm}
    \begin{itemize}
        \item \texttt{/rawdata} --- original data, \textbf{never modify}
        \vspace{0.1cm}
        \item \texttt{/workdata} --- cleaned/processed data
        \vspace{0.1cm}
        \item \texttt{/code} --- all scripts (\texttt{.do}, \texttt{.R}, \ldots)
        \vspace{0.1cm}
        \item \texttt{/output} --- tables, figures, logs
    \end{itemize}
    \vspace{0.2cm}
    \item Use \textbf{relative paths} or a \textbf{path setup block}
    at the top of every script so the project runs on any machine
    \vspace{0.2cm}
    \item \textbf{Never overwrite raw data} --- all cleaning steps should
    be recorded in a script and saved to \texttt{/workdata}
    \vspace{0.2cm}
    \item \textbf{Comment your code} --- your future self (and collaborators)
    will thank you
    \vspace{0.2cm}
    % \begin{itemize}
    %     \item AI tools (e.g.\ ChatGPT, Claude) can help you
    %     \textbf{write, explain, and debug} Stata code efficiently
    % \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item \textbf{Read the codebook} before running anything:
    \vspace{0.1cm}
    \begin{itemize}
        \item What does each variable measure? What are the units?
        \vspace{0.1cm}
        \item How is the treatment defined? What is the outcome?
        \vspace{0.1cm}
        \item Are there known data issues (top-coding, imputation, \ldots)?
    \end{itemize}
    \vspace{0.2cm}
    \item \textbf{Label your variables and values} for clarity:
    \vspace{0.1cm}
    \begin{itemize}
        \item Give every variable a descriptive name and unit
        \vspace{0.1cm}
        \item Label categorical values (e.g., $0 =$ Control, $1 =$ Treated)
    \end{itemize}
    \vspace{0.2cm}
    \item \textbf{Explore the data} systematically:
    \vspace{0.1cm}
    \begin{itemize}
        \item Examine variable types, distributions, and frequencies
        \vspace{0.1cm}
        \item Check for \textbf{missing values}, \textbf{outliers},
        and \textbf{implausible values}
    \end{itemize}
\end{itemize}
\end{frame}


\begin{frame}{How AI Agents Can Help}{}
\begin{itemize}
    \item \textbf{Agent-type AI} (e.g.\ Claude Code, Gemini CLI) can operate
    directly in your project folder --- not just answer questions, but
    \textbf{read, write, and execute code} on your behalf
    % \vspace{0.2cm}
    % \item What you can ask an AI agent to do:
    % \vspace{0.1cm}
    % \begin{itemize}
    %     \item \textbf{Set up the project}: create folder structure,
    %     write path setup block, initialize scripts
    %     \vspace{0.1cm}
    %     \item \textbf{Explore the data}: summarize variables, flag missing
    %     values, outliers, and implausible values --- and report back to you
    %     \vspace{0.1cm}
    %     \item \textbf{Clean the data}: generate labeling and cleaning code
    %     based on the codebook you provide
    %     \vspace{0.1cm}
    %     \item \textbf{Debug}: read your error messages and fix the code
    %     directly in the file
    %     \vspace{0.1cm}
    %     \item \textbf{Translate}: convert a Stata \texttt{.do} file into R,
    %     or vice versa
    %     \vspace{0.1cm}
    %     \item \textbf{Document}: add comments and variable descriptions
    %     throughout your scripts
    % \end{itemize}
    % \vspace{0.2cm}
    % \item \textbf{Key difference from chatbots}: agents take actions
    % autonomously across multiple steps --- you describe the goal,
    % the agent figures out the steps
\end{itemize}
% \begin{center}
% \small \textbf{Important:} Review what the agent did before proceeding ---
% always stay in control of your own data and code.
% \end{center}
\end{frame}
...
\begin{frame}[fragile]{R Example}{Step 3: PSM Estimation via \texttt{MatchIt}}
\begin{lstlisting}
library(MatchIt)
...
\begin{itemize}
    \vspace{0.2cm}
    \item \textbf{matchit()}: estimates PS via logistic regression
    and performs nearest neighbor matching in one step
    \vspace{0.2cm}
    \item \textbf{distance = "logit"}: uses logit model to estimate
    propensity scores
    \vspace{0.2cm}
    \item \textbf{replace = TRUE}: matching with replacement
\end{itemize}
\end{frame}


\begin{frame}[fragile]{R Example}{Step 3: Estimate ATT}
\begin{lstlisting}
...
\begin{itemize}
    \vspace{0.2cm}
    \item \textbf{match.data()}: extracts matched sample with
    \texttt{MatchIt} weights
    \vspace{0.2cm}
    \item \textbf{lm()}: estimates ATT on the matched sample;
    \texttt{weights} is automatically created by \texttt{match.data()}
    \vspace{0.2cm}
    \item \textbf{Advantage over \texttt{Match()}}: \texttt{MatchIt}
    provides a unified framework for PSM, CEM, and other methods
...
\begin{itemize}
    \item PSM reduces matching to one dimension (the propensity score),
    but requires a correctly specified model
    \vspace{0.2cm}
    \item \textbf{CEM} takes a more direct approach:
    \vspace{0.2cm}
    \begin{itemize}
        \item \textbf{Coarsen} each covariate into discrete bins
        (e.g., age 20--29, 30--39, \ldots)
        \vspace{0.2cm}
        \item \textbf{Exact match} on the coarsened values: a treated
        and a control unit are matched only if they fall in the
        \emph{same bin for every covariate simultaneously}
        \vspace{0.2cm}
        \item A treated and a control unit that cannot be matched
        across \emph{all} covariates simultaneously are
        \textbf{discarded}
    \end{itemize}
    \vspace{0.2cm}
    \item No propensity score model needed --- covariate balance is
    \textbf{guaranteed by construction}
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item[1] \textbf{Coarsen}: bin each continuous covariate $X_k$
    into discrete intervals (automatic or user-defined cutpoints)
    \vspace{0.2cm}
    \begin{itemize}
        \item E.g., age: $[20,30)$, $[30,40)$, $[40,50)$;
        earnings: $[0, 5\text{K})$, $[5\text{K}, 15\text{K})$, \ldots
    \end{itemize}
    \vspace{0.3cm}
    \item[2] \textbf{Exact match} on coarsened values:
    \vspace{0.2cm}
    \begin{itemize}
        \item A treated and a control unit are matched only if they
        fall in the \emph{same bin for every covariate simultaneously}
        \vspace{0.2cm}
        \item Each such group of matched units is called a
        \textbf{stratum}: a subgroup of ``similar'' units who fall
        in the same bin for every covariate
        \vspace{0.2cm}
        \item Units that cannot be matched across \emph{all}
        covariates simultaneously are \textbf{discarded}
    \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{How CEM Works}{Stratum Example}

\begin{center}
\small
Each \textbf{stratum} = a unique combination of coarsened bins.\\
Units are matched \emph{only within} the same stratum.
\end{center}
...
\begin{tabular}{ccccc}
\toprule
\textbf{Stratum} & \textbf{Age bin} & \textbf{Earnings bin} & \textbf{Treated ($D=1$)} & \textbf{Control ($D=0$)} \\
\midrule
1 & $[20,30)$ & $[0, 5\text{K})$   & 2 units & 3 units $\checkmark$ \\
2 & $[20,30)$ & $[5\text{K},15\text{K})$ & 1 unit  & 0 units $\rightarrow$ \textcolor{red}{discarded} \\
3 & $[30,40)$ & $[0, 5\text{K})$   & 0 units & 2 units $\rightarrow$ \textcolor{red}{discarded} \\
4 & $[30,40)$ & $[5\text{K},15\text{K})$ & 3 units & 4 units $\checkmark$ \\
%5 & $[40,50)$ & $[5\text{K},15\text{K})$ & 1 unit  & 2 units $\checkmark$ \\
\bottomrule
\end{tabular}}
\end{center}
...
\vspace{0.3cm}
\begin{itemize}
    \item Stratam 1, 4: both treated and control units present
    $\Rightarrow$ \textbf{matched}
    \vspace{0.1cm}
    \item Stratum 2: no control units available
    $\Rightarrow$ treated unit \textcolor{red}{discarded}
    \vspace{0.1cm}
    \item Stratum 3: no treated units
    $\Rightarrow$ control units \textcolor{red}{discarded}
\end{itemize}
...
\begin{itemize}
    \item[3] For each \textbf{matched} treated unit $i$, impute the
    counterfactual using the \textbf{average outcome of control units
    in the same stratum} $s(i)$:
    \begin{equation*}
        \hat{\alpha}_{\text{ATT}} =
        \dfrac{1}{N_1}\sum_{D_i=1}\!\left(Y_i -
        \bar{Y}_{0,s(i)}\right)
    \end{equation*}
    \begin{itemize}
        \item $N_1$: number of \textbf{matched} treated units
        (unmatched treated units are discarded)
        \vspace{0.2cm}
        \item $s(i)$: the stratum that treated unit $i$ belongs to
        \vspace{0.2cm}
        \item $\bar{Y}_{0,s(i)} =
        \dfrac{1}{N_{0,s(i)}}\!\displaystyle\sum_{D_j=0,\,j\in
        s(i)}\!Y_j$: average \emph{observed} outcome of control
        units ($D_j=0$) in $i$'s stratum
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item \textbf{No model dependence}: no propensity score to
    estimate, so no misspecification risk
    \vspace{0.2cm}
    \item \textbf{Guaranteed balance}: treated and matched controls
    fall in the same covariate bins by construction
    \vspace{0.2cm}
    \item \textbf{Transparent}: the coarsening is explicit and
    easy to interpret
    \vspace{0.2cm}
    \item \textbf{Tradeoff}:
    \vspace{0.2cm}
    \begin{itemize}
        \item Finer bins $\Rightarrow$ better balance but fewer
        matched units
        \vspace{0.2cm}
        \item Coarser bins $\Rightarrow$ more matches but looser
        balance
        \vspace{0.2cm}
        \item With many covariates, most strata may be empty ---
...
\begin{frame}[fragile]{CEM: R Example}{Run CEM}
\begin{lstlisting}
...
\end{lstlisting}
\begin{itemize}
    \item \textbf{matchit()}: coarsens covariates and creates strata;
    covariates in the formula are used for matching
\end{itemize}
\end{frame}

\begin{frame}[fragile]{CEM: R Example}{Estimate ATT}
\begin{lstlisting}
...
\end{lstlisting}
\begin{itemize}
    \item \textbf{match.data()}: extracts the matched sample with
    CEM weights
    \item \textbf{lm()}: estimates ATT on the matched sample using
    CEM weights; \texttt{weights} is automatically created by
    \texttt{match.data()}
\end{itemize}
\end{frame}
