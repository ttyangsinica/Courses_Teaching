...
\begin{frame}{From Causal Effect to Counterfactual}
	\begin{itemize}
		...
		\vspace{0.3cm}
		\item Causal effects are defined by \textbf{potential outcomes}. For example, the ATT:
		\begin{equation*}
		...
		\end{equation*}
		\vspace{0.2cm}
		\item The first term is observed --- it is the average outcome of the treated group
		\vspace{0.3cm}
		\item The second term, ${\color{red}\mathrm{E}[\mathrm{Y}^{0}_{i} \mid D_i=1]}$, is the \textbf{counterfactual}: what would the treated group's outcome have been \emph{had they not been treated}?
		\vspace{0.3cm}
...
\begin{frame}{ODO = ATT + Selection Bias}
	\begin{itemize}
	...
	\end{equation*}
	\begin{itemize}
		\item \textbf{Selection Bias} $= {\color{red}\mathrm{E}[\mathrm{Y}^{0}_{i}|D_i=1]} - \mathrm{E}[\mathrm{Y}^{0}_{i}|D_i=0]$: the difference in \emph{untreated} potential outcomes between the two groups
		\vspace{0.2cm}
...
\end{frame}
	...
	\begin{frame}{An RCT in the Wild: Noy \& Zhang (2023, \textit{Science})}
		\begin{itemize}
			\item \textbf{Research Question:}
			\vspace{0.2cm}
			\begin{itemize}
				\item Does access to ChatGPT raise productivity for mid-level professional writing tasks?
			\end{itemize}
			\vspace{0.3cm}
			\item \textbf{Setting:} Preregistered online experiment with 453 college-educated professionals (marketers, grant writers, consultants, data analysts, HR professionals, managers)
			\vspace{0.3cm}
			\item \textbf{Tasks:} Two occupation-specific writing assignments (press releases, short reports, analysis plans, delicate emails) --- 20--30 minute tasks designed to resemble real work
			\vspace{0.3cm}
			\item \textbf{Incentives:} Base pay \$10 + up to \$14 bonus for output quality; evaluators were blinded, experienced professionals grading on a 1--7 scale
		\end{itemize}
	\end{frame}
	
	\begin{frame}{Experimental Design}
		\begin{itemize}
			\item Both groups completed a \textbf{pre-treatment Task 1} without ChatGPT
			\vspace{0.3cm}
			\item \textbf{Treatment ($D_i = 1$):} Instructed to register for ChatGPT and permitted to use it on Task 2
			\vspace{0.3cm}
			\item \textbf{Control ($D_i = 0$):} Instructed to register for LaTeX editor Overleaf --- same hassle cost, but no AI access; $<5\%$ used Overleaf on the task
			\vspace{0.4cm}
			\item This is a \textbf{between-subjects RCT}: 50\% randomly assigned to each group ($n_1 = 218$, $n_0 = 235$)
			\vspace{0.3cm}
			\item \textbf{Outcomes ($Y_i$):}
			\vspace{0.2cm}
			\begin{itemize}
				\item Time spent on Task 2 (minutes)
				\vspace{0.1cm}
				\item Average evaluator grade on Task 2 (1--7 scale)
			\end{itemize}
		\end{itemize}
	\end{frame}
	
	\begin{frame}{Step 1: Check Balance --- Why It Matters}
		\begin{itemize}
	...
	\begin{frame}{Step 1: How to Test Balance}
		\begin{itemize}
	...
	\begin{frame}{Step 1: Balance Table --- Continuous Variables}
		\begin{center}
			\begin{threeparttable}
				\linespread{1}
				...
				\begin{tabular}{@{}lcccccc@{}}
					\toprule
					Variable & \multicolumn{2}{c}{Treatment ($n=218$)} & \multicolumn{2}{c}{Control ($n=235$)} & Diff. & $p$-value \\
					& Mean & (SD) & Mean & (SD) & & \\
					\midrule\midrule
					...
					\midrule\midrule
					Observations & \multicolumn{2}{c}{218} & \multicolumn{2}{c}{235} & & \\
					\bottomrule
				\end{tabular}
				\begin{tablenotes}
					\fontsize{7}{7pt}\selectfont
					\item Source: Noy \& Zhang (2023), Table 1. SD not reported in original; shown as (--).
					\item $p$-values from two-sample $t$-test using standard formula. Salary SE approximated.
				\end{tablenotes}
			\end{threeparttable}
		\end{center}
	\end{frame}
	
	\begin{frame}{Step 1: Balance --- Continuous Variables (Discussion)}
		\begin{itemize}
			\item All continuous pre-treatment variables are \textbf{well balanced}:
			\vspace{0.3cm}
			\begin{itemize}
				\item Annual salary: Diff.\ $=\$4{,}174$, $t = 1.14$, $p = 0.256$ --- not significant
				\vspace{0.2cm}
				\item Pre-treatment Task 1 grade: Diff.\ $= 0.14$, $t = 1.17$, $p = 0.244$ --- not significant
				\vspace{0.2cm}
				\item Task 1 time spent: Diff.\ $= 0.48$ min, $t = 0.41$, $p = 0.681$ --- not significant
				\vspace{0.2cm}
				\item All other variables also show $|t| < 1.96$ and $p > 0.05$
			\end{itemize}
			\vspace{0.4cm}
			\item \textbf{Implication:} For continuous variables, the two groups appear similar \emph{before} treatment --- consistent with successful randomization
		\end{itemize}
	\end{frame}
	
	\begin{frame}{Step 1: Balance Table --- Binary Variables}
		\begin{center}
			\begin{threeparttable}
				\linespread{1}
				\fontsize{8}{8pt}\selectfont
				...
				\begin{tabular}{@{}lcccccc@{}}
					...
					& Mean & (SD) & Mean & (SD) & & \\
					\midrule\midrule
					Employed & 0.960 & (0.196) & 0.910 & (0.286) & 0.050 & \textcolor{red}{0.013**} \\
					\midrule
					Occ: HR Professional & 0.110 & (0.314) & 0.060 & (0.238) & 0.046 & \textcolor{red}{0.028*} \\
					Occ: Consultant & 0.110 & (0.314) & 0.130 & (0.337) & $-0.013$ & 0.655 \\
					Occ: Data Analyst & 0.110 & (0.314) & 0.110 & (0.314) & 0.000 & 1.000 \\
					Occ: Grant Writer & 0.170 & (0.376) & 0.160 & (0.367) & 0.012 & 0.724 \\
					Occ: Manager & 0.410 & (0.493) & 0.430 & (0.496) & $-0.017$ & 0.719 \\
					Occ: Marketer & 0.090 & (0.287) & 0.110 & (0.314) & $-0.023$ & 0.412 \\
					\midrule\midrule
					Observations & \multicolumn{2}{c}{218} & \multicolumn{2}{c}{235} & & \\
					\bottomrule
				\end{tabular}
				\begin{tablenotes}
					\fontsize{7}{7pt}\selectfont
					\item Source: Noy \& Zhang (2023), Table 1. SD computed as $\sqrt{\hat{p}(1-\hat{p})}$.
	...
	\begin{frame}{Step 1: Balance --- Binary Variables (Discussion)}
		\begin{itemize}
			\item \textbf{Good news:} 11 out of 13 pre-treatment variables are well balanced ($p > 0.05$)
			\vspace{0.3cm}
			\item \textbf{Two exceptions} (highlighted in red):
			\vspace{0.2cm}
			\begin{itemize}
				\item \textbf{Employment status:} Treatment 96\% vs. Control 91\%, $t = 2.50$, $p = 0.013$
				\vspace{0.2cm}
				\item \textbf{HR occupation:} Treatment 11\% vs. Control 6\%, $t = 2.19$, $p = 0.028$
			\end{itemize}
			\vspace{0.3cm}
			\item With 13 variables tested simultaneously, we expect about 0.65 spurious rejections at 5\% level by chance alone --- so two is not alarming
			\vspace{0.3cm}
			\item \textbf{Authors' response:} Robustness checks controlling for employment status and occupation confirm the main results are unaffected
			\vspace{0.3cm}
			\item \textbf{Overall:} Randomization is largely successful; the ODO is a credible estimate of the ATE
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Step 2: Main Results --- ODO as Causal Effect}
		\begin{center}
			\begin{threeparttable}
				...
				\begin{tabular}{@{}lcccccc@{}}
					\toprule
					Outcome (Task 2) & \multicolumn{2}{c}{Treatment ($n=218$)} & \multicolumn{2}{c}{Control ($n=235$)} & Diff.\ (ODO) & $p$-value \\
					& Mean & (SD) & Mean & (SD) & & \\
					\midrule\midrule
					Time Spent (min) & 17.0 & ($\approx$17.3) & 27.0 & ($\approx$17.3) & \textcolor{red}{$-10.0$} & \textcolor{red}{$<0.001$***} \\
					Average Grade (1--7) & 4.53 & ($\approx$1.37) & 3.80 & ($\approx$1.37) & \textcolor{red}{$+0.73$} & \textcolor{red}{$<0.001$***} \\
					\midrule\midrule
					Observations & \multicolumn{2}{c}{218} & \multicolumn{2}{c}{235} & & \\
					\bottomrule
				\end{tabular}
				\begin{tablenotes}
					\fontsize{7}{7pt}\selectfont
					\item Source: Noy \& Zhang (2023), Figure 1. SD approximated from reported effect sizes ($-0.75$ SDs for time; $+0.41$ SDs for grade).
					\item Diff.\ $=$ ODO $=$ ATE under RCT. *** $p<0.001$. Treatment effect estimated via regression with occupation $\times$ task-order fixed effects.
				\end{tablenotes}
			\end{threeparttable}
		\end{center}
	\end{frame}
