...
\item[\textbullet] You may use \textbf{either Stata or R}. Submit \textbf{two files}:
  \begin{enumerate}
    \item \textbf{Empirical analysis record (HTML)}: A single HTML file documenting your entire empirical workflow from loading the data through the analysis in Questions 5--12.
    \begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Answer sheet (PDF)}: A PDF answering all questions (0--12). For Questions 5--12, copy key output and figures from your HTML record and add written interpretation.
  \end{enumerate}
\item[\textbullet] Recommended R packages: \texttt{haven}, \texttt{dplyr}, \texttt{ggplot2}, \texttt{fixest}, \texttt{rmarkdown}.
\item[\textbullet] Upload files here: \textbf{\url{https://www.dropbox.com/request/tHC1fXI6jlvN060Oezs1}}
...
\subsection*{Question 0 \quad Empirical Analysis Record}

Submit an HTML file recording your complete empirical workflow --- from loading the data to the analyses in Questions 5--12. The HTML should include:
\begin{itemize}
...
\begin{description}
  \item[Hint (Stata):] Create a \texttt{.stmd} file with markdown text and Stata code blocks, then compile:
  ...
  \item[Hint (R):] Create an \texttt{.Rmd} file with YAML header \texttt{output: html\_document} and embed R code in \texttt{\`{}\`{}\`{}\{r\}} chunks. Render with:
...
\begin{enumerate}[(a)]
  \item What is the \textbf{research question}? (e.g., ``Does X causally affect Y?'')
  \item What is your \textbf{treatment variable} ($D$)? How is it defined and measured in your data?
  \item What is your \textbf{outcome variable} ($Y$)? How is it defined and measured in your data?
\end{enumerate}
...
\begin{enumerate}[(a)]
  ...
  \item Briefly describe \textbf{three} data-cleaning commands you used and explain what each one does.
\end{enumerate}

\begin{description}
  \item[Hint:] Useful commands include:
    \begin{itemize}
...
Create a graph displaying the \textbf{distribution} of a variable you are interested in. Briefly explain your findings.

\begin{description}
  \item[Hint (Stata):] \texttt{histogram varname, xtitle("...") ytitle("...")}
  \item[Hint (R):] \texttt{ggplot(df, aes(x = var)) + geom\_histogram() + labs(x = "...", y = "...")}
\end{description}
...
Create a graph displaying the \textbf{relationship between two variables} of interest (e.g., your treatment and outcome). Briefly explain your findings.

\begin{description}
  \item[Hint (Stata):] \texttt{graph twoway (scatter y x) (lfit y x)}
  \item[Hint (R):] \texttt{ggplot(df, aes(x = x, y = y)) + geom\_point() + geom\_smooth(method = "lm")}
\end{description}
...
\begin{description}
  \item[Hint (Stata):] \texttt{regress y treat}
  \item[Hint (R):] \texttt{lm(y \textasciitilde treat, data = df)}
\end{description}
...
\begin{description}
  \item[Hint (Stata):] \texttt{regress y treat controls}
  \item[Hint (R):] \texttt{lm(y \textasciitilde treat + controls, data = df)}
\end{description}
...
\begin{description}
  \item[Hint (Stata):] \texttt{reghdfe y treat controls, absorb(fe1 fe2)}
  \item[Hint (R):] \texttt{fixest::feols(y \textasciitilde treat + controls | fe1 + fe2, data = df)}
\end{description}
