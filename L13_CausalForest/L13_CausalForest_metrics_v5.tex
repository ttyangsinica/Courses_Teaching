...
\begin{itemize}
  \item Understand what problem causal forest solves.
  \vspace{0.2cm}
  \item Learn just enough decision tree and random forest intuition to understand causal forest.
  \vspace{0.2cm}
  \item Know what assumptions are needed for causal interpretation.
  \vspace{0.2cm}
  \item Read the main outputs: ATE, CATE distribution, BLP, variable importance.
  \vspace{0.2cm}
  \item See how to run a simple example using the \texttt{grf} package in R.
\end{itemize}
\end{frame}

\begin{frame}{One-Sentence Summary}
\begin{block}{Causal forest}
  A causal forest estimates how the treatment effect varies with observed characteristics $X$ by averaging many honest causal trees.
\end{block}
...
\vspace{0.2cm}
\begin{itemize}
  \item It is not just a prediction tool.
  \item It is a way to learn \textbf{who benefits more} from a policy or program.
\end{itemize}
\end{frame}

\begin{frame}{Connection to Earlier Lectures}
\begin{itemize}
  \item Earlier lectures mostly focused on one average causal parameter:
  \[
    \alpha_{\text{ATE}}
    =
  ...
  \]
  \vspace{0.2cm}
  \item Some policy questions focus on treated units:
  \[
    \alpha_{\text{ATT}}
    =
  ...
  \]
  \vspace{0.2cm}
  \item This lecture asks a more granular question:
  \[
    \alpha_{\text{CATE}}(x)
    =
...
\]
\begin{itemize}
  \item One number for the whole sample.
  \item Useful, but it hides heterogeneity.
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Conditional Average Treatment Effect}
\[
  \tau(x)
  =
  \mathrm{E}\!\left[\mathrm{Y}^{1}_{i}-\mathrm{Y}^{0}_{i}\mid X_i=x\right]
\]
\begin{itemize}
  \item A treatment effect for people with characteristics $x$.
  \item Useful for targeting and mechanism.
\end{itemize}
\end{column}
\end{columns}
...
\vspace{0.4cm}
\begin{center}
\textbf{In the causal forest literature, CATE is usually written as $\tau(x)$.}
\end{center}
\end{frame}

\begin{frame}{How ATE and ATT Relate to CATE}
\begin{itemize}
  \item Once we know the CATE function $\tau(x)$, familiar estimands are averages of it:
  \[
    \alpha_{\text{ATE}}
    =
  ...
  \]
  \vspace{0.2cm}
  \item Therefore, causal forest does not replace ATE or ATT.
  \vspace{0.1cm}
  \begin{itemize}
    \item It estimates a richer object first, then we can average it for policy summaries.
  \end{itemize}
\end{itemize}
\end{frame}
...
\vspace{0.35cm}
\begin{itemize}
  \item The average effect may be positive.
  \item But policy decisions often require knowing \textbf{where} the effect is large.
...
\begin{itemize}
  ...
  \]
  \vspace{0.15cm}
  \item But we only observe one of them:
  \[
    \mathrm{Y}_i
    =
    D_i\mathrm{Y}^{1}_{i}
...
\vspace{0.25cm}
\begin{block}{So causal forest does not magically observe individual effects.}
It estimates average effects for people with similar $X$.
\end{block}
\end{frame}
...
\vspace{0.2cm}
\begin{itemize}
  \item After controlling for observed covariates $X_i$, treatment is as good as random.
  \vspace{0.2cm}
  \item This is automatically credible in a randomized experiment.
  \vspace{0.2cm}
  \item In observational data, it requires rich covariates and careful design.
  \vspace{0.2cm}
  \item Causal forest is a powerful estimator, not a substitute for identification.
\end{itemize}
\end{frame}

\begin{frame}{Identification of CATE}
\begin{itemize}
  \item Under conditional independence, we can write:
  \[
    \tau(x)
    =
  ...
  \]
  \vspace{0.2cm}
  \item This is the same treated-control comparison as before, but now conditional on $X_i=x$.
  \vspace{0.2cm}
  \item The hard part is that $X_i$ can have many variables, so exact cells become too small.
  \vspace{0.2cm}
  \item Causal forest uses trees to find useful local comparisons automatically.
\end{itemize}
\end{frame}
