...
\begin{frame}[fragile]{Reproducible Research: \texttt{.stmd} File Structure (1)}
	...
	\begin{itemize}
		...
		\vspace{0.2cm}
		\item \textbf{Quiet block} (\texttt{```s/q}): code runs but \textbf{nothing is shown} --- use for paths and data setup
	\end{itemize}
...
\begin{frame}[fragile]{Reproducible Research: \texttt{.stmd} File Structure (2)}
	...
	\begin{itemize}
		\item \textbf{Regular block} (\texttt{```s}): shows \textbf{both code and Stata output} in the document
		\vspace{0.2cm}
		\item \textbf{Inline value} (\texttt{\textasciigrave s \textit{expr}\textasciigrave}): embed a computed result directly in a sentence
	\end{itemize}
...
\end{lstlisting}
	...
	\begin{itemize}
		\item The inline expression \texttt{\textasciigrave s \%12.0fc \_b[college]\textasciigrave} inserts the estimated coefficient directly into the sentence --- updates automatically when the model changes
		\vspace{0.1cm}
...
\begin{frame}[fragile]{Reproducible Research: Key Syntax Elements}
	...
	\begin{tabular}{lll}
		...
		\hline
		\texttt{```s} \ldots \texttt{```}   & Show code \textbf{and} output  & regressions, summary stats \\
		\texttt{```s/q} \ldots \texttt{```} & Run quietly, \textbf{nothing shown} & paths, data cleaning \\
		\texttt{\textasciigrave s \textit{expr}\textasciigrave} & Embed value \textbf{inline} in text & report coefficients \\
		\hline
	\end{tabular}
...
\end{lstlisting}
	...
	\vspace{0.2cm}
	\begin{itemize}
		\item The inline expression \texttt{`s \%12.0fc \_b[college]`} inserts the estimated coefficient directly into the sentence --- updates automatically when data or model changes
	\end{itemize}
...
\begin{frame}[fragile]{Reproducible Research: \texttt{.Rmd} File Structure (2)}
	...
	\begin{itemize}
		...
		\vspace{0.2cm}
		\item \textbf{Inline value} (\texttt{\textasciigrave r \textit{expr}\textasciigrave}): embed a computed result directly in a sentence
	\end{itemize}
...
\end{lstlisting}
	...
	\begin{itemize}
		\item The inline expression \texttt{\textasciigrave r round(coef(model3)["college"], 0)\textasciigrave} inserts the estimated coefficient directly into the sentence --- updates automatically when data or model changes
	\end{itemize}
...
\begin{frame}[fragile]{Reproducible Research: Key Syntax Elements}
	...
	\begin{tabular}{lll}
		...
		Syntax & What it does & Use for \\
		\hline
		\texttt{\`\`\`\{r\}} \ldots \texttt{\`\`\`}              & Show code \textbf{and} output  & regressions, summary stats \\
		\texttt{\`\`\`\{r, include=FALSE\}} \ldots \texttt{\`\`\`} & Run quietly, \textbf{nothing shown} & paths, data loading \\
		\texttt{\`r \textit{expr}\`}                               & Embed value \textbf{inline} in text & report coefficients \\
		\hline
	\end{tabular}%
	}
