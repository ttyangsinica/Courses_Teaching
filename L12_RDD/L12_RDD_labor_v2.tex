...
% ================================================================
% Stata/R Example: Fuzzy RDD
% Based on Bleemer & Mehta (2022) using simulated data
% ================================================================
...
\begin{frame}{Stata Exercise: Bleemer \& Mehta (2022)}{Overview}
	\begin{itemize}
		\item We use \textbf{simulated data} calibrated to Bleemer \& Mehta (2022)
		\vspace{0.2cm}
		\begin{itemize}
			\item Original data are administrative records (not publicly available)
			\vspace{0.2cm}
			\item Simulation replicates key features of the paper:
			first-stage $\approx$ 36 pp; LATE $\approx$ 46\% wage premium (\$22,000/year)
		\end{itemize}
		\vspace{0.2cm}
		\item \textbf{Dataset: \texttt{simulated\_econ\_rdd.dta}} -- key variables:
		\vspace{0.2cm}
		\begin{itemize}
			\item \texttt{gpa\_econ}: GPA in Econ 1 \& 2 -- running variable $A_i$
			\item \texttt{above}: $Z_i = \mathbf{1}[A_i \geq 2.8]$ -- eligibility for Econ major
			\item \texttt{econ\_major}: Economics major -- \textbf{fuzzy} treatment $D_i$
			\item \texttt{logwage\_1718}: log wages 2017-2018 -- outcome $Y_i$
			\item \texttt{female}, \texttt{urm}, \texttt{sat\_score}, \texttt{zip\_income}: pre-determined covariates
		\end{itemize}
		\vspace{0.2cm}
		\item See \texttt{exercise\_stata\_econ\_rdd.do} and \texttt{exercise\_r\_econ\_rdd.R}
	\end{itemize}
\end{frame}


\begin{frame}{Stata Exercise: Bleemer \& Mehta (2022)}{STATA Implementation}
	\begin{itemize}
		\item Use \texttt{simulated\_econ\_rdd.dta} (generated by \texttt{simulate\_econ\_rdd.do})
		\vspace{0.2cm}
		...
		\end{itemize}
		\vspace{0.2cm}
		\item \textbf{Key difference from sharp RDD}: We need \textbf{two} graphs and \textbf{three} regressions:
		\begin{itemize}
			\vspace{0.2cm}
			\item \textbf{First stage}: $D_i$ vs $A_i$ (does GPA 2.8 shift major choice?)
			\item \textbf{Reduced form}: $Y_i$ vs $A_i$ (does GPA 2.8 shift wages?)
			\item \textbf{Fuzzy IV} (2SLS): LATE = Reduced Form / First Stage
		\end{itemize}
	\end{itemize}
\end{frame}


\begin{frame}{Stata: Step 1}{Graphical Analysis -- First Stage}
	\begin{itemize}
		\item In fuzzy RDD, \textbf{always} plot both the first stage and reduced form
		\item First stage: does crossing GPA 2.8 shift economics enrollment ($D_i$)?
	\end{itemize}
\begin{lstlisting}
use "simulated_econ_rdd.dta", clear
* First stage: P(Economics Major) by GPA around cutoff
...
\begin{frame}[fragile]{Stata: Step 1}{Graphical Analysis -- Reduced Form and Balance}
	\begin{itemize}
		\item Reduced form: does GPA 2.8 shift wages ($Y_i$)?
		\item Covariate balance: \texttt{sat\_score}, \texttt{female} should show \textbf{no} jump
	\end{itemize}
\begin{lstlisting}
* Reduced form: log wages by GPA
binscatter logwage_1718 gpa_econ if employed_1718, n(50) rd(2.8) linetype(lfit)  /*
*/ xtitle("GPA in Introductory Economics")  /*
*/ ytitle("Log Wages 2017-2018")            /*
*/ title("Reduced Form: GPA Cutoff and Wages")
* Covariate balance: SAT score (no jump expected)
...
\begin{frame}[fragile]{Stata: Step 2}{Manipulation / Sorting Test}
	\begin{itemize}
		\item If students can manipulate GPA to just exceed 2.8, density spikes just above cutoff
		\item $H_0$: no density discontinuity at cutoff (non-rejection $\Rightarrow$ RDD valid)
	\end{itemize}
\begin{lstlisting}
* McCrary (2008) density test
DCdensity gpa_econ, breakpoint(2.8)            /*
*/  generate(Xj Yj r0 fhat se_fhat)
drop Xj Yj r0 fhat se_fhat
* Alternative: rddensity (Cattaneo, Jansson, Ma 2020)
rddensity gpa_econ, c(2.8)
\end{lstlisting}
\end{frame}


\begin{frame}[fragile]{Stata: Step 3}{Parametric Estimation -- First Stage and Reduced Form}
	\begin{itemize}
		\item Create centered running variable, estimate first stage and reduced form
	\end{itemize}
\begin{lstlisting}
* Centered running variable and interaction terms
gen gpa_c       = gpa_econ - 2.8
gen gpa_c2      = gpa_c^2
gen above_gpa_c = above * gpa_c
gen above_gpa_c2 = above * gpa_c2
* First stage: effect of eligibility (Z_i=above) on treatment (D_i=econ_major)
reg econ_major above gpa_c above_gpa_c, cl(gpa_c)
* Reduced form: effect of eligibility on log wages (Intent-to-Treat)
reg logwage_1718 above gpa_c above_gpa_c if employed_1718, cl(gpa_c)
* Manual Wald estimate: LATE = Reduced Form / First Stage
\end{lstlisting}
\end{frame}


\begin{frame}[fragile]{Stata: Step 3}{Parametric Estimation -- Fuzzy IV (2SLS)}
	\begin{itemize}
		\item Instrument $D_i$ (\texttt{econ\_major}) with $Z_i$ (\texttt{above})
		\item Compare with OLS (biased upward due to ability selection)
	\end{itemize}
\begin{lstlisting}
* Fuzzy RDD: 2SLS
* Syntax: (econ_major = above) instruments econ_major with above
ivregress 2sls logwage_1718 gpa_c above_gpa_c  /*
*/    (econ_major = above) if employed_1718, vce(cl gpa_c)
* OLS (biased -- for comparison only)
* Unobserved ability -> both major choice and wages (selection bias)
reg logwage_1718 econ_major gpa_c above_gpa_c  /*
*/    if employed_1718, cl(gpa_c)
* OLS coefficient on econ_major should be > LATE (upward bias)
\end{lstlisting}
\end{frame}


\begin{frame}[fragile]{Stata: Step 4}{Nonparametric Estimation -- rdrobust}
	\begin{itemize}
		\item \texttt{rdrobust} with \texttt{fuzzy()} computes LATE directly with optimal bandwidth
	\end{itemize}
\begin{lstlisting}
...
* Fuzzy RDD: LATE
* fuzzy() option specifies actual treatment D_i
rdrobust logwage_1718 gpa_econ if employed_1718,  /*
*/    c(2.8) fuzzy(econ_major) kernel(tri) all
* Formal covariate balance tests (all p-values should be > 0.05)
...
\begin{frame}{Stata: Interpreting Results}{Key Findings from Simulated Data}
	\begin{itemize}
		\item \textbf{First stage}: $\approx +0.36$ -- economics enrollment rises 36 pp at the cutoff
		\vspace{0.2cm}
		\item \textbf{Reduced form}: $\approx +0.14$ -- wages rise $\sim$14\% at the cutoff (intent-to-treat)
		\vspace{0.2cm}
		\item \textbf{LATE} (fuzzy RDD / 2SLS): $\approx +0.38$ log points ($\approx$ 46\% higher wages)
		\vspace{0.2cm}
		\begin{itemize}
			\item Manual check: LATE = $0.14 / 0.36 \approx 0.38$ \checkmark
			\vspace{0.2cm}
		\end{itemize}
		\item \textbf{OLS}: $>0.38$ (biased upward due to ability selection)
		\vspace{0.2cm}
		\begin{itemize}
			\item High-ability students are more likely to choose economics AND earn more
			\vspace{0.2cm}
			\item OLS picks up both the causal effect \textit{and} the selection effect
		\end{itemize}
		\vspace{0.2cm}
		\item Results match the paper: Bleemer \& Mehta (2022) find LATE $\approx$ 46\% (\$22,000/year)
	\end{itemize}
\end{frame}
...
rdplot(y = data_emp$logwage_1718, x = data_emp$gpa_econ, c = cutoff,
       ...
       x.label = "GPA in Introductory Economics",
       y.label = "Log Average Wages (2017-18)")
# Covariate balance: SAT score (no jump expected)
...
\begin{frame}[fragile]{R: Step 2}{Manipulation Test}
\begin{lstlisting}[language=R]
# rddensity: H0 = no density discontinuity at GPA = 2.8
...
\begin{frame}[fragile]{R: Step 3}{Parametric Estimation}
\begin{lstlisting}[language=R]
# First stage
...
# Fuzzy IV (2SLS): instrument econ_major with above
fuzzy_iv <- ivreg(
...
\begin{frame}[fragile]{R: Step 4}{Nonparametric Estimation -- rdrobust}
\begin{lstlisting}[language=R]
# First stage
summary(rdrobust(data$econ_major, data$gpa_econ, c = cutoff, kernel = "tri", all = TRUE))
# Reduced form
summary(rdrobust(data_emp$logwage_1718, data_emp$gpa_econ, c = cutoff, kernel = "tri", all = TRUE))
# Fuzzy RDD: LATE
# fuzzy = D_i (actual treatment); rdrobust instruments with Z_i automatically
rdd_fuzzy <- rdrobust(y     = data_emp$logwage_1718,
                      x     = data_emp$gpa_econ,
                      fuzzy = data_emp$econ_major,
                      c = cutoff, kernel = "tri", all = TRUE)
summary(rdd_fuzzy)
# Covariate balance tests (all should have p > 0.05)
for (v in c("sat_score", "female", "urm", "zip_income")) {
  rdd_b <- rdrobust(data[[v]], data$gpa_econ, c = cutoff, kernel = "tri")
  cat(v, ": coef =", round(rdd_b$coef[1], 4), " p =", round(rdd_b$pv[1], 4), "\n")
}
\end{lstlisting}
\end{frame}
