...
\begin{frame}{From Prediction to Counterfactual Prediction}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\vspace{0.2cm}
			\item Under the CIA, the predicted counterfactual is valid
			\vspace{0.2cm}
...
\begin{frame}{Review: Omitted Variable Bias}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item Covariance between $\mathrm{Y}_{i}$ and $D_{i}$: $\text{Cov}(\mathrm{Y}_i, D_i) = \frac{1}{N} \sum^N_{i=1} (Y_i - \bar{Y})(D_i - \bar{D})$
			\vspace{0.2cm}
			\item Variance of $D_{i}$: $V(D_i) = \frac{1}{N} \sum^N_{i=1} (D_i - \bar{D})^2$
		\end{itemize}
		
		\vspace{0.3cm}
		\item Failure to include enough (right) control variables in the regression would result in bias
		\vspace{0.3cm}
...
\begin{frame}{From OVB to Controlling for Covariates}
	\begin{itemize}
		\item We saw that OVB arises when $X_i$ is correlated with $D_i$ and affects $Y_i$
		\vspace{0.3cm}
		\item The solution: \textbf{partial out} the influence of $X_i$ from treatment $D_i$
		\vspace{0.3cm}
		\begin{itemize}
			\item Isolate the variation in $D_i$ that is \textbf{unrelated to} $X_i$
			\vspace{0.2cm}
			\item Use only that ``clean'' variation to estimate the effect on $Y_i$
		\end{itemize}
		\vspace{0.3cm}
		\item This logic is formalized by the \textbf{Frisch-Waugh-Lovell (FWL) Theorem}
	\end{itemize}
\end{frame}
       ...
       \begin{frame}{Review: Frisch-Waugh-Lovell Theorem}{Main Idea}
       	\begin{itemize}
       		...
       		\item The FWL Theorem states that the following OLS estimators for $\alpha$ are equivalent:
       		\vspace{0.2cm}
       		\begin{enumerate}
...
\begin{frame}{Bad Control Problem and Selection Bias}{Example}
	\begin{itemize} 
		\item Suppose master degree completion is \textbf{randomly assigned}
		\vspace{0.2cm}
		\item Now consider comparing earnings \textbf{within white collar workers}:
		\vspace{0.2cm}
		\begin{itemize} 
			\item Group A: Has a master degree \textbf{and} works white collar
			\vspace{0.2cm}
			\item Group B: No master degree, \textbf{but still} works white collar
			\vspace{0.2cm}
		\end{itemize}
			\item Group A is a \textbf{selected} sample --- among those randomly assigned a master degree, only \textbf{some} end up in white collar jobs
		\vspace{0.2cm}
        \item Group B is a \textbf{selected} sample --- to obtain a white collar job without a master degree, these individuals likely have \textbf{higher unobserved ability}
		\vspace{0.2cm}
		\item Conditioning on occupation \textbf{creates} a new form of selection bias, even when treatment was randomly assigned
	\end{itemize}
\end{frame}
...
\begin{frame}{Distribution of t-statistic}{Visualizing the $p$-value}
	\begin{center}
		\includegraphics[width=0.90\textwidth]{figure/pvalue_employed.png}
	\end{center}
	\vspace{0.1cm}
	%\begin{itemize}
...
\begin{frame}{Interpretation of Regression Results}{Heterogeneous Treatment Effects}
	\begin{itemize}
...
\begin{frame}{Reproducible Research: What is \texttt{markstat}?}

	\begin{itemize}
		\item \textbf{markstat} is a Stata package for \textbf{reproducible research}
		\vspace{0.3cm}
		\begin{itemize}
			\item Write Markdown text and Stata code together in \textbf{one file} (extension: \texttt{.stmd})
			\vspace{0.2cm}
			\item Compile to produce a \textbf{formatted document} with code, output, and explanation interleaved
			\vspace{0.2cm}
			\item Output format: \textbf{HTML} (open in browser) or \textbf{PDF} (requires \LaTeX)
		\end{itemize}
		\vspace{0.3cm}
		\item Compared to a regular \texttt{.do} file:
		\vspace{0.2cm}
		\begin{itemize}
			\item A \texttt{.do} file only produces a log file with raw text output
			\vspace{0.2cm}
			\item A \texttt{.stmd} file produces a \textbf{readable, shareable document} with section headings, formatted tables, and narrative explanation
		\end{itemize}
	\end{itemize}
...
\begin{frame}[fragile]{Reproducible Research: Installation (One-Time Setup)}

	\begin{itemize}
		\item \textbf{Step 1: Install Pandoc} --- an external program (not a Stata package)
		\vspace{0.2cm}
		\begin{itemize}
			\item Download and install from \texttt{pandoc.org}
			\vspace{0.1cm}
			\item Pandoc converts Markdown to HTML/PDF; markstat calls it internally
		\end{itemize}
	\end{itemize}
	...
	\begin{itemize}
		\item \textbf{Step 2: Install markstat in Stata}
	\end{itemize}
...
\end{lstlisting}
	...
	\begin{itemize}
		\item \textbf{Step 3: Tell Stata where Pandoc is installed}
	\end{itemize}

\begin{lstlisting}
* Adjust path to match your Pandoc installation:
whereis pandoc "C:\Program Files\Pandoc\pandoc.exe"
\end{lstlisting}

	\begin{itemize}
		\item Steps 1--3 are done \textbf{only once}
	\end{itemize}
...
\begin{frame}[fragile]{Reproducible Research: Compiling a \texttt{.stmd} File}

	\begin{itemize}
		\item \textbf{Step 4: Write your document} and save as \texttt{regression.stmd}
		\vspace{0.2cm}
		\begin{itemize}
			\item Use any plain text editor: Notepad, VS Code, or Stata's do-file editor
			\vspace{0.1cm}
			\item The full file \texttt{regression.stmd} is available on the course dropbox
		\end{itemize}
		\vspace{0.3cm}
		\item \textbf{Step 5: Compile} from Stata
	\end{itemize}
...
\end{lstlisting}
	...
	\begin{itemize}
		\item \texttt{bundle} packs everything (tables, graphs) into a single \texttt{.html} file --- easy to share
	\end{itemize}
...
\begin{frame}[fragile]{Reproducible Research: \texttt{.stmd} File Structure (1)}

	\begin{itemize}
		\item A \texttt{.stmd} file starts with a \textbf{YAML header} (title, author, date), followed by \textbf{Markdown text} and \textbf{Stata code blocks}
		\vspace{0.2cm}
		\item \textbf{Quiet block} (\texttt{{\char96}{\char96}{\char96}s/q}): code runs but \textbf{nothing is shown} --- use for paths and data setup
	\end{itemize}

\begin{lstlisting}[language={},basicstyle=\ttfamily\fontsize{8}{10}\selectfont,keywordstyle={}]
---
title: "Regression Analysis with CPS Data"
author: "Tzu-Ting Yang, Academia Sinica"
date: "Causal Data Course"
---

## 1. Data Setup

```s/q
* Runs silently: paths and data loading not shown in output
global rawdata = "C:\...\rawdata"
...
\begin{frame}[fragile]{Reproducible Research: \texttt{.stmd} File Structure (2)}

	\begin{itemize}
		\item \textbf{Regular block} (\texttt{{\char96}{\char96}{\char96}s}): shows \textbf{both code and Stata output} in the document
		\vspace{0.2cm}
		\item \textbf{Inline value} (\texttt{{\char96} s \textit{expr}{\char96}}): embed a computed result directly in a sentence
	\end{itemize}

\begin{lstlisting}[language={},basicstyle=\ttfamily\fontsize{7.5}{9}\selectfont,keywordstyle={},breaklines=true,breakatwhitespace=true]
## 2. OLS Regression

```s
* Code and full Stata output both appear in document
reg incwage college health_1-health_4 age i.race, vce(robust)
```

The college wage premium is `s %12.0fc _b[college]` USD per year.
\end{lstlisting}

	\begin{itemize}
		\item The inline expression \texttt{{\char96} s \%12.0fc \_b[college]{\char96}} inserts the estimated coefficient directly into the sentence --- updates automatically when the model changes
		% \vspace{0.1cm}
		% \item Full file \texttt{regression.stmd} is available on the course website
	\end{itemize}
...
\begin{frame}[fragile]{Reproducible Research: Key Syntax Elements}

	\begin{itemize}
		\item Three types of code blocks / inline expressions:
	\end{itemize}
	...
	\begin{tabular}{lll}
		...
		\hline
		\texttt{{\char96}{\char96}{\char96}s} \ldots \texttt{{\char96}{\char96}{\char96}}   & Show code \textbf{and} output  & regressions, summary stats \\
		\texttt{{\char96}{\char96}{\char96}s/q} \ldots \texttt{{\char96}{\char96}{\char96}} & Run quietly, \textbf{nothing shown} & paths, data cleaning \\
		\texttt{{\char96} s \textit{expr}{\char96}} & Embed value \textbf{inline} in text & report coefficients \\
		\hline
	\end{tabular}

% 	\vspace{0.4cm}

% \begin{lstlisting}[language={},basicstyle=\ttfamily\fontsize{7.5}{9}\selectfont,keywordstyle={}]
% ```s
% reg incwage college health_1-health_4 age i.race, vce(robust)
% ```
...
% 	\vspace{0.2cm}
% 	\begin{itemize}
% 		\item The inline expression \texttt{{\char96}s \%12.0fc \_b[college]{\char96}} inserts the estimated coefficient directly into the sentence --- updates automatically when data or model changes
% 	\end{itemize}
...
\begin{frame}{Reproducible Research: What is R Markdown?}

	\begin{itemize}
		\item \textbf{R Markdown} is a file format (\texttt{.Rmd}) for reproducible research in R
		\vspace{0.3cm}
		\begin{itemize}
			\item Write R code and narrative text together in \textbf{one file}
			\vspace{0.2cm}
			\item Compile to produce a formatted \textbf{HTML, PDF, or Word} document with code, output, and explanation interleaved
		\end{itemize}
		\vspace{0.3cm}
		\item Compared to a regular \texttt{.R} script:
		\vspace{0.2cm}
		\begin{itemize}
			\item A \texttt{.R} script only produces console output
			\vspace{0.2cm}
			\item A \texttt{.Rmd} file produces a \textbf{readable, shareable document} with section headings, formatted tables, and equations
		\end{itemize}
		\vspace{0.3cm}
		\item Built into \textbf{RStudio} --- no extra installation of external programs needed
	\end{itemize}
...
\begin{frame}[fragile]{Reproducible Research: Installation and Compile}

	\begin{itemize}
		\item \textbf{Installation} (one-time, in R console):
	\end{itemize}
...
\end{lstlisting}
	...
	\begin{itemize}
		\item \textbf{Compile} in RStudio: open \texttt{regression.Rmd} and click the \textbf{Knit} button
		\vspace{0.2cm}
		\item Or compile from the R console:
	\end{itemize}

\begin{lstlisting}
rmarkdown::render("regression.Rmd")          # HTML output
rmarkdown::render("regression.Rmd",
  output_format = "pdf_document")            # PDF (requires LaTeX)
\end{lstlisting}
	\begin{itemize}
		\item Output file (\texttt{regression.html}) is saved in the same folder
		\vspace{0.2cm}
		\item The full file \texttt{regression.Rmd} is available on the course dropbox
	\end{itemize}
...
\begin{frame}[fragile]{Reproducible Research: \texttt{.Rmd} File Structure (1)}

	\begin{itemize}
		\item A \texttt{.Rmd} file starts with a \textbf{YAML header}, followed by Markdown text and R code chunks
		\vspace{0.2cm}
		\item The \textbf{setup chunk} (\texttt{include=FALSE}): runs silently --- use for paths and package loading
	\end{itemize}
...
library(haven);  library(estimatr);  library(dplyr)
rawdata <- "C:/nest/Dropbox/.../rawdata"
acs_2015 <- read_dta(paste0(rawdata, "/cps_2014_16.dta"))
```
\end{lstlisting}
...
\begin{frame}[fragile]{Reproducible Research: \texttt{.Rmd} File Structure (2)}

	\begin{itemize}
		\item \textbf{Regular chunk} (\texttt{\{r\}}): shows \textbf{both code and R output} in the document
		\vspace{0.2cm}
		\item \textbf{Inline value} (\texttt{{\char96} r \textit{expr}{\char96}}): embed a computed result directly in a sentence
	\end{itemize}
...
```{r model3}
model3 <- lm_robust(incwage ~ college + health_1 + health_2 +
           health_3 + health_4 + age + factor(race),
           data = acs_2015, se_type = "HC1")
summary(model3)
```

The college wage premium is
`r round(coef(model3)["college"], 0)` USD per year.
\end{lstlisting}

	\begin{itemize}
		\item The inline expression \texttt{{\char96} r round(coef(model3)["college"], 0){\char96}} inserts the estimated coefficient directly into the sentence 
        
        %--- updates automatically when data or model changes
	\end{itemize}
...
\begin{frame}[fragile]{Reproducible Research: Key Syntax Elements}
	...
	\begin{itemize}
		\item Three types of code chunks / inline expressions:
	\end{itemize}
	...
	\begin{tabular}{lll}
		...
		Syntax & What it does & Use for \\
		\hline
		\texttt{{\char96}{\char96}{\char96}\{r\}} \ldots \texttt{{\char96}{\char96}{\char96}}              & Show code \textbf{and} output  & regressions, summary stats \\
		\texttt{{\char96}{\char96}{\char96}\{r, include=FALSE\}} \ldots \texttt{{\char96}{\char96}{\char96}} & Run quietly, \textbf{nothing shown} & paths, data loading \\
		\texttt{{\char96}r \textit{expr}{\char96}}                  & Embed value \textbf{inline} in text & report coefficients \\
		\hline
	\end{tabular}%
	}
	...
	\begin{itemize}
		\item \texttt{echo=FALSE}: hides code, shows output 
        		\vspace{0.2cm}
        \item \texttt{include=FALSE}: hides both
		% \vspace{0.2cm}
		% \item Full file \texttt{regression.Rmd} is available on the course website
	\end{itemize}
