...
\begin{itemize} 
\item What is the effect of increasing the minimum wage on employment?
\vspace{0.2cm} 
\item Minimum wage is effective only in certain jobs: 
\vspace{0.2cm} 
\begin{itemize} 
...
\begin{frame}{DID Estimation}{Use Regression to Get DID Estimator}
\begin{itemize}
	\item Estimate the DID estimator in a regression framework has the following advantages: 
	\vspace{0.2cm}
	\begin{itemize}
		\item It is easy to calculate standard errors
		\vspace{0.2cm}
		\item We can control for other variables which may reduce the selection bias further
		\vspace{0.2cm}
		\item It is easy to include multiple periods
		\vspace{0.2cm}
		\item We can study treatments with different \textbf{treatment intensity} (continous measure)
			\vspace{0.2cm}
			\begin{itemize}	
		
		\item Example: varying increases in the minimum wage for different states
					\vspace{0.2cm}
		\item Estimate equation: \\ $Y_{it} = \alpha + \delta (\text{TreatmentIntensity}_{it} \times Post_t) + \lambda_i + \gamma_t + \epsilon_{it}$
		 	\end{itemize}
	\end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}{Dynamic DID Design}{Example}
	...
	Hsing-Wen Han, Kuang-Ta Lo, Yung-Yu Tsai, and Tzu-Ting Yang (2023), ``\textbf{The Effect of Financial Resources on Fertility: Evidence from Administrative Data on Lottery Winners}", Working Paper
...
\begin{frame}{Empirical Example: Han et al. (2023)}{Motivation}
	\begin{itemize} 
...
\begin{frame}{Empirical Example: Han et al. (2023)}{Dynamic DID Design}
	\begin{itemize} 
...
\begin{frame}[fragile]{Step 3: Pre/Post DID regression}{STATA Command: outreg2}
\begin{lstlisting}	
reg work post treated treated_post,r
outreg2 using "$table\DID_pre_post.xls", replace nocon keep(treated_post)  ///
stats(coef se) addstat(Sample Size, e(N)) ///
addtext(Basic, Yes, Age, No, Demo, No, State FE, No, Year FE, No)
\end{lstlisting}
	\begin{itemize}
		\item \textbf{outreg2:} Stata command (from a user-written package) to export regression tables to an external file
		\begin{itemize}
						\vspace{0.2cm}
			\item \textbf{using "\$table\textbackslash DID\_pre\_post.xls":} Path and filename for the output Excel file (tex/csv)
						\vspace{0.2cm}
			\item \textbf{replace:} Overwrites any existing file with the same name
						\vspace{0.2cm}
			\item \textbf{nocon:} Suppresses the constant term in the output table
						\vspace{0.2cm}
				\item \textbf{keep(treated\_post):} Only output the coefficient for the treated\_post variable
...
\begin{frame}[fragile]{Step 3: Pre/Post DID regression}{STATA Command: outreg2}
\begin{lstlisting}	
reg work post treated treated_post age age2,r
outreg2 using "$table\DID_pre_post.xls", append nocon keep(treated_post)  ///
stats(coef se) addstat(Sample Size, e(N)) ///
addtext(Basic, Yes, Age, Yes, Demo, No, State FE, No, Year FE, No)
\end{lstlisting}
	\begin{itemize}
		\item \textbf{outreg2 options:}
		\begin{itemize}
								\vspace{0.2cm}
			\item \textbf{append:} Append the output to an existing file
						\vspace{0.2cm}
			\item \textbf{stats(coef se):} Specifies to report coefficients and standard errors
			\vspace{0.2cm}
			\item \textbf{addstat(Sample Size, e(N)):} Adds the sample size to the table using e(N) which returns the number of observations
			\vspace{0.2cm}
			\item \textbf{addtext():} Adds text to the table describing which controls were included in the model
			\vspace{0.2cm}
			\item \textbf{///:} Line continuation marker in Stata, allowing the command to continue on the next line
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{figure}[p]
	\centering
\includegraphics[width=0.9\textwidth]{figure/stata_did_table1.png} 
%	\includegraphics{stata_did_table1.png}
\end{figure}
...
\begin{frame}[fragile]{Step 3: Pre/Post DID regression}{R Command}
	\begin{itemize}
		\item Show the treatment effect by treatment intensity:
\begin{lstlisting}
DID <- lm(work ~ treated_post+post+treated, data = eitc_data)
