...
\begin{itemize}
    \item Identification tells us what IV estimates:
    \[
    \alpha_{IV}
    =
    E[Y_i^1-Y_i^0 \mid D_i^1>D_i^0]
    \]
    \item Estimation tells us how to estimate it in data:
    \[
    \hat{\alpha}_{IV}
    =
    ...
    \]
    \item Therefore, TSLS is a practical way to estimate LATE using sample data.
\end{itemize}
\end{frame}
...
\begin{frame}{Review: IV Estimation}{Two Stage Least Squares (TSLS)}
	...
	\item In practice,  We can estimate $\alpha_{IV}$ by using Two Stage Least Squares (TSLS).
	% \vspace{0.2cm}

	% \item TSLS is especially useful when we include covariates, $X_i$, or have multiple instruments.
	\vspace{0.2cm}
	...
	\begin{itemize}
		\item[1.] \textbf{First stage:} predict the part of treatment explained by the instrument
		\begin{equation*}
			D_i = \kappa + \alpha_{FS} Z_i + X_i' \beta + \nu_i.
		\end{equation*}
		Estimate this equation and obtain fitted values $\hat{D}_i$.

		\vspace{0.2cm}

		\item[2.] \textbf{Second stage:} regress the outcome on the predicted treatment
		\begin{equation*}
	...
	\item Intuitively, TSLS uses only the variation in $D_i$ that is generated by the instrument $Z_i$.
	\end{itemize}
\end{frame}


\begin{frame}{Review: IV Estimation: How TSLS connects to LATE}
	\begin{itemize}
		\item The first stage isolates the variation in military service that comes from the lottery draft.
		\vspace{0.2cm}

		\item This is exactly the variation that distinguishes compliers from others:
		\[
		D_i^1 > D_i^0.
		\]
		...
		\item Therefore, TSLS does not estimate the average treatment effect for everyone.
		\vspace{0.2cm}

		\item Under the IV assumptions, TSLS estimates:
		\[
		E[Y_i^1-Y_i^0 \mid D_i^1>D_i^0],
		\]
		the average causal effect for compliers.

		\vspace{0.2cm}

		% \item In short:
		% \[
		% \text{TSLS is the estimation method; LATE is the causal parameter being estimated.}
		% \]
	\end{itemize}
\end{frame}
...
% \begin{frame}{Review: IV Estimation}{Two Stage Least Squares (TSLS)}
% 	\begin{itemize}
% 	\item In practice, we should not manually run the two stages and then use the usual OLS standard errors from the second stage.
% 	\vspace{0.3cm}

% 	\item Why? Because the second-stage regressor $\hat{D}_i$ is estimated from the first stage.
% 	\vspace{0.3cm}

% 	\item If we treat $\hat{D}_i$ as if it were observed data, the standard errors will be incorrect:
% 	\[
% 	u_i^*
% 	=
% 	Y_i - \delta - X_i'\gamma - \alpha_{TSLS}\hat{D}_i.
% 	\]

% 	\vspace{0.3cm}

% 	\item The correct IV standard errors account for the fact that $\hat{D}_i$ is generated from the first-stage regression.
% 	\vspace{0.3cm}

% 	\item Standard software packages, such as Stata and R, automatically compute the correct IV/TSLS standard errors.
% 	\end{itemize}
% \end{frame}
...
\begin{frame}{Testing for Weak Instruments}
	\begin{itemize}
		\item \textbf{First-stage F-statistic} (homoskedastic case):
				\vspace{0.2cm}
		\begin{itemize}
			\item Cragg-Donald F-statistic from first-stage regression
					\vspace{0.2cm}
...
\begin{frame}{Statistical Inference Robust to Weak Instruments}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Use \texttt{ivreg2} or \texttt{ivreghdfe} with the \texttt{first} option.
			\vspace{0.2cm}

			\item Example without high-dimensional fixed effects:
			\begin{equation*}
			\texttt{ivreg2 Y X (D = Z), robust first}
			\end{equation*}

			\item Example with high-dimensional fixed effects:
			\begin{equation*}
			\texttt{ivreghdfe Y X (D = Z), absorb(fe) cluster(id) first}
			\end{equation*}

			\item The output reports weak-IV diagnostics, including:
			\begin{itemize}
				\item Cragg-Donald Wald F statistic
				\item Kleibergen-Paap rk LM statistic
				\item Kleibergen-Paap rk Wald F statistic
			\end{itemize}

			\vspace{0.2cm}

			\item With robust or clustered standard errors, focus on the Kleibergen-Paap rk Wald F statistic rather than the Cragg-Donald statistic.
		\end{itemize}
	\end{itemize}
\end{frame}
